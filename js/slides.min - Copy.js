if (window.pluginsAttached) { alert("Attention: plugins.js library is attached twice and needs to be removed. The slides.min.js already contains both of scripts."); } else { window.pluginsAttached = 1; }(function($) { $.extend($, { cacheImage: function(src, options) { if (typeof src === "object") { $.each(src, function() { $.cacheImage(String(this), options); }); return; } var image = new Image();
            options = options || {};
            $.each(["load", "error", "abort"], function() { var e = String(this); if (typeof options[e] === "function") { $(image).bind(e, options[e]); } if (typeof options.complete === "function") { $(image).bind(e, options.complete); } });
            image.src = src; return image; } });
    $.extend($.fn, { cacheImage: function(options) { return this.each(function() { $.cacheImage(this.src, options); }); } }); })(jQuery);
...
! function(a) { "function" == typeof define && define.amd ? define(["jquery"], a) : "object" == typeof exports ? module.exports = a : a(jQuery); }(function(a) {
    ...
    function d(a, b) { return k.settings.adjustOldDeltas && "mousewheel" === a.type && b % 120 === 0; } var e, f, g = ["wheel", "mousewheel", "DOMMouseScroll", "MozMousePixelScroll"],
        ...
        i = Array.prototype.slice; if (a.event.fixHooks) { for (var j = g.length; j;) { a.event.fixHooks[g[--j]] = a.event.mouseHooks; } } var k = a.event.special.mousewheel = { version: "3.1.12", setup: function() { if (this.addEventListener) { for (var c = h.length; c;) { this.addEventListener(h[--c], b, !1); } } else { this.onmousewheel = b; }
...
$.fn.removeClassByPrefix = function(prefix) { this.each(function(i, el) { var classes = el.className.split(" ").filter(function(c) { return c.lastIndexOf(prefix, 0) !== 0; });
        el.className = $.trim(classes.join(" ")); }); return this; };
! function($, e) { var t = {};
    t.eventName = "resizeEnd", t.delay = 250, t.poll = function() { var n = $(this),
            a = n.data(t.eventName);
        a.timeoutId && window.clearTimeout(a.timeoutId), a.timeoutId = window.setTimeout(function() { n.trigger(t.eventName); }, t.delay); }, $.event.special[t.eventName] = { setup: function() { var e = $(this);
            e.data(t.eventName, {}), e.on("resize", t.poll); }, teardown: function() { var n = $(this),
                a = n.data(t.eventName);
            a.timeoutId && window.clearTimeout(a.timeoutId), n.removeData(t.eventName), n.off("resize", t.poll); } }, $.fn[t.eventName] = function(e, n) { return arguments.length > 0 ? this.on(t.eventName, null, e, n) : this.trigger(t.eventName); }; }(jQuery, this);
(function(a) { if (typeof define === "function" && define.amd && define.amd.jQuery) { define(["jquery"], a); } else { a(jQuery); } }(function(f) { var p = "left",
    ...
    function C(a4, av) { var az = (a || d || !av.fallbackToMouseEvents),
        ...
        function O(bd, bb) { var bc = undefined; if (I() || V()) { bc = aF(bd, bb, l); } else { if ((P() || aX()) && bc !== false) { bc = aF(bd, bb, t); } } if (aG() && bc !== false) { bc = aF(bd, bb, j); } else { if (ap() && bc !== false) { bc = aF(bd, bb, b); } else { if (ah() && bc !== false) { bc = aF(bd, bb, A); } } } if (bb === q) { a9(bd); } if (bb === h) { if (a) { if (bd.touches.length == 0) { a9(bd); } } else { a9(bd); } } return bc; }
        ...
        function al(bb, bc) { if (av.allowPageScroll === m || aX()) { bb.preventDefault(); } else { var bd = av.allowPageScroll === s; switch (bc) {
                    case p:
...
;
(function(g, i, j, b) { var h = "sharrre",
        f = { className: "sharrre", share: { googlePlus: false, facebook: false, twitter: false, digg: false, delicious: false, stumbleupon: false, linkedin: false, pinterest: false }, shareTotal: 0, template: "", title: "", url: j.location.href, text: j.title, urlCurl: "sharrre.php", count: {}, total: 0, shorterTotal: true, enableHover: true, enableCounter: true, enableTracking: false, hover: function() {}, hide: function() {}, click: function() {}, render: function() {}, buttons: { googlePlus: { url: "", urlCount: false, size: "medium", lang: "en-US", annotation: "" }, facebook: { url: "", urlCount: false, action: "like", layout: "button_count", width: "", send: "false", faces: "false", colorscheme: "", font: "", lang: "en_US" }, twitter: { url: "", urlCount: false, count: "horizontal", hashtags: "", via: "", related: "", lang: "en" }, digg: { url: "", urlCount: false, type: "DiggCompact" }, delicious: { url: "", urlCount: false, size: "medium" }, stumbleupon: { url: "", urlCount: false, layout: "1" }, linkedin: { url: "", urlCount: false, counter: "" }, pinterest: { url: "", media: "", description: "", layout: "horizontal" } } },
        c = { googlePlus: "", facebook: "https://graph.facebook.com/fql?q=SELECT%20url,%20normalized_url,%20share_count,%20like_count,%20comment_count,%20total_count,commentsbox_count,%20comments_fbid,%20click_count%20FROM%20link_stat%20WHERE%20url=%27{url}%27&callback=?", twitter: "http://cdn.api.twitter.com/1/urls/count.json?url={url}&callback=?", digg: "http://services.digg.com/2.0/story.getInfo?links={url}&type=javascript&callback=?", delicious: "http://feeds.delicious.com/v2/json/urlinfo/data?url={url}&callback=?", stumbleupon: "", linkedin: "http://www.linkedin.com/countserv/count/share?format=jsonp&url={url}&callback=?", pinterest: "http://api.pinterest.com/v1/urls/count.json?url={url}&callback=?" },
        l = { googlePlus: function(m) { var n = m.options.buttons.googlePlus;
                ...
                i.___gcfg = { lang: m.options.buttons.googlePlus.lang }; var o = 0; if (typeof gapi === "undefined" && o == 0) { o = 1;
                    (function() { var p = j.createElement("script");
                        p.type = "text/javascript";
                        p.async = true;
                        p.src = "//apis.google.com/js/plusone.js"; var q = j.getElementsByTagName("script")[0];
                        q.parentNode.insertBefore(p, q); })(); } else { gapi.plusone.go(); } }, facebook: function(m) { var n = m.options.buttons.facebook;
                g(m.element).find(".buttons").append('<div class="button facebook"><div id="fb-root"></div><div class="fb-like" data-href="' + (n.url !== "" ? n.url : m.options.url) + '" data-send="' + n.send + '" data-layout="' + n.layout + '" data-width="' + n.width + '" data-show-faces="' + n.faces + '" data-action="' + n.action + '" data-colorscheme="' + n.colorscheme + '" data-font="' + n.font + '" data-via="' + n.via + '"></div></div>'); var o = 0; if (typeof FB === "undefined" && o == 0) { o = 1;
                    (function(t, p, u) { var r, q = t.getElementsByTagName(p)[0]; if (t.getElementById(u)) { return; }
                        r = t.createElement(p);
                        r.id = u;
                        r.src = "//connect.facebook.net/" + n.lang + "/all.js#xfbml=1";
                        q.parentNode.insertBefore(r, q); }(j, "script", "facebook-jssdk")); } else { FB.XFBML.parse(); } }, twitter: function(m) { var n = m.options.buttons.twitter;
                g(m.element).find(".buttons").append('<div class="button twitter"><a href="https://twitter.com/share" class="twitter-share-button" data-url="' + (n.url !== "" ? n.url : m.options.url) + '" data-count="' + n.count + '" data-text="' + m.options.text + '" data-via="' + n.via + '" data-hashtags="' + n.hashtags + '" data-related="' + n.related + '" data-lang="' + n.lang + '">Tweet</a></div>'); var o = 0; if (typeof twttr === "undefined" && o == 0) { o = 1;
                    (function() { var q = j.createElement("script");
                        q.type = "text/javascript";
                        q.async = true;
                        q.src = "//platform.twitter.com/widgets.js"; var p = j.getElementsByTagName("script")[0];
                        p.parentNode.insertBefore(q, p); })(); } else { g.ajax({ url: "//platform.twitter.com/widgets.js", dataType: "script", cache: true }); } }, digg: function(m) { var n = m.options.buttons.digg;
                g(m.element).find(".buttons").append('<div class="button digg"><a class="DiggThisButton ' + n.type + '" rel="nofollow external" href="http://digg.com/submit?url=' + encodeURIComponent((n.url !== "" ? n.url : m.options.url)) + '"></a></div>'); var o = 0; if (typeof __DBW === "undefined" && o == 0) { o = 1;
                    (function() { var q = j.createElement("SCRIPT"),
                            p = j.getElementsByTagName("SCRIPT")[0];
                        q.type = "text/javascript";
                        q.async = true;
                        q.src = "//widgets.digg.com/buttons.js";
                        p.parentNode.insertBefore(q, p); })(); } }, delicious: function(o) { if (o.options.buttons.delicious.size == "tall") { var p = "width:50px;",
                        n = "height:35px;width:50px;font-size:15px;line-height:35px;",
                        m = "height:18px;line-height:18px;margin-top:3px;"; } else { var p = "width:93px;",
                        n = "float:right;padding:0 3px;height:20px;width:26px;line-height:20px;",
                        m = "float:left;height:20px;line-height:20px;"; } var q = o.shorterTotal(o.options.count.delicious); if (typeof q === "undefined") { q = 0; }
                ...
                g(o.element).find(".delicious").on("click", function() { o.openPopup("delicious"); }); }, stumbleupon: function(m) { var n = m.options.buttons.stumbleupon;
                g(m.element).find(".buttons").append('<div class="button stumbleupon"><su:badge layout="' + n.layout + '" location="' + (n.url !== "" ? n.url : m.options.url) + '"></su:badge></div>'); var o = 0; if (typeof STMBLPN === "undefined" && o == 0) { o = 1;
                    (function() { var p = j.createElement("script");
                        p.type = "text/javascript";
                        p.async = true;
                        p.src = "//platform.stumbleupon.com/1/widgets.js"; var q = j.getElementsByTagName("script")[0];
                        q.parentNode.insertBefore(p, q); })();
                    s = i.setTimeout(function() { if (typeof STMBLPN !== "undefined") { STMBLPN.processWidgets();
                            clearInterval(s); } }, 500); } else { STMBLPN.processWidgets(); } }, linkedin: function(m) { var n = m.options.buttons.linkedin;
                g(m.element).find(".buttons").append('<div class="button linkedin"><script type="in/share" data-url="' + (n.url !== "" ? n.url : m.options.url) + '" data-counter="' + n.counter + '"><\/script></div>'); var o = 0; if (typeof i.IN === "undefined" && o == 0) { o = 1;
                    (function() { var p = j.createElement("script");
                        p.type = "text/javascript";
                        p.async = true;
                        p.src = "//platform.linkedin.com/in.js"; var q = j.getElementsByTagName("script")[0];
                        q.parentNode.insertBefore(p, q); })(); } else { i.IN.init(); } }, pinterest: function(m) { var n = m.options.buttons.pinterest;
                ...
                (function() { var o = j.createElement("script");
                    o.type = "text/javascript";
                    o.async = true;
                    o.src = "//assets.pinterest.com/js/pinit.js"; var p = j.getElementsByTagName("script")[0];
                    p.parentNode.insertBefore(o, p); })(); } },
        d = { googlePlus: function() {}, facebook: function() { fb = i.setInterval(function() { if (typeof FB !== "undefined") { FB.Event.subscribe("edge.create", function(m) { _gaq.push(["_trackSocial", "facebook", "like", m]); });
                        ...
                        clearInterval(tw); } }, 1000); }, digg: function() {}, delicious: function() {}, stumbleupon: function() {}, linkedin: function() {
                function m() { _gaq.push(["_trackSocial", "linkedin", "share"]); } }, pinterest: function() {} },
        a = { googlePlus: function(m) { i.open("https://plus.google.com/share?hl=" + m.buttons.googlePlus.lang + "&url=" + encodeURIComponent((m.buttons.googlePlus.url !== "" ? m.buttons.googlePlus.url : m.url)), "", "toolbar=0, status=0, width=900, height=500"); }, facebook: function(m) { i.open("http://www.facebook.com/sharer/sharer.php?u=" + encodeURIComponent((m.buttons.facebook.url !== "" ? m.buttons.facebook.url : m.url)) + "&t=" + m.text + "", "", "toolbar=0, status=0, width=900, height=500"); }, twitter: function(m) { i.open("https://twitter.com/intent/tweet?text=" + encodeURIComponent(m.text) + "&url=" + encodeURIComponent((m.buttons.twitter.url !== "" ? m.buttons.twitter.url : m.url)) + (m.buttons.twitter.via !== "" ? "&via=" + m.buttons.twitter.via : ""), "", "toolbar=0, status=0, width=650, height=360"); }, digg: function(m) { i.open("http://digg.com/tools/diggthis/submit?url=" + encodeURIComponent((m.buttons.digg.url !== "" ? m.buttons.digg.url : m.url)) + "&title=" + m.text + "&related=true&style=true", "", "toolbar=0, status=0, width=650, height=360"); }, delicious: function(m) { i.open("http://www.delicious.com/save?v=5&noui&jump=close&url=" + encodeURIComponent((m.buttons.delicious.url !== "" ? m.buttons.delicious.url : m.url)) + "&title=" + m.text, "delicious", "toolbar=no,width=550,height=550"); }, stumbleupon: function(m) { i.open("http://www.stumbleupon.com/badge/?url=" + encodeURIComponent((m.buttons.delicious.url !== "" ? m.buttons.delicious.url : m.url)), "stumbleupon", "toolbar=no,width=550,height=550"); }, linkedin: function(m) { i.open("https://www.linkedin.com/cws/share?url=" + encodeURIComponent((m.buttons.delicious.url !== "" ? m.buttons.delicious.url : m.url)) + "&token=&isFramed=true", "linkedin", "toolbar=no,width=550,height=550"); }, pinterest: function(m) { i.open("http://pinterest.com/pin/create/button/?url=" + encodeURIComponent((m.buttons.pinterest.url !== "" ? m.buttons.pinterest.url : m.url)) + "&media=" + encodeURIComponent(m.buttons.pinterest.media) + "&description=" + m.buttons.pinterest.description, "pinterest", "toolbar=no,width=700,height=300"); } };
    ...
    function k(n, m) { this.element = n;
        ...
        this._name = h;
        this.init(); }
    k.prototype.init = function() { var m = this; if (this.options.urlCurl !== "") { c.googlePlus = this.options.urlCurl + "?url={url}&type=googlePlus";
            c.stumbleupon = this.options.urlCurl + "?url={url}&type=stumbleupon"; }
        g(this.element).addClass(this.options.className); if (typeof g(this.element).data("title") !== "undefined") { this.options.title = g(this.element).attr("data-title"); } if (typeof g(this.element).data("url") !== "undefined") { this.options.url = g(this.element).data("url"); } if (typeof g(this.element).data("text") !== "undefined") { this.options.text = g(this.element).data("text"); }
        ...
        g(this.element).hover(function() { if (g(this).find(".buttons").length === 0 && m.options.enableHover === true) { m.loadButtons(); }
            m.options.hover(m, m.options); }, function() { m.options.hide(m, m.options); });
        g(this.element).click(function() { m.options.click(m, m.options); return false; }); };
    k.prototype.loadButtons = function() { var m = this;
        g(this.element).append('<div class="buttons"></div>');
        g.each(m.options.share, function(n, o) { if (o == true) { l[n](m); if (m.options.enableTracking === true) { d[n](); } } }); };
    k.prototype.getSocialJson = function(o) { var m = this,
            p = 0,
            n = c[o].replace("{url}", encodeURIComponent(this.options.url)); if (this.options.buttons[o].urlCount === true && this.options.buttons[o].url !== "") { n = c[o].replace("{url}", this.options.buttons[o].url); } if (n != "" && m.options.urlCurl !== "") { g.getJSON(n, function(r) { if (typeof r.count !== "undefined") { var q = r.count + "";
                    q = q.replace("\u00c2\u00a0", "");
                    p += parseInt(q, 10); } else { if (r.data && r.data.length > 0 && typeof r.data[0].total_count !== "undefined") { p += parseInt(r.data[0].total_count, 10); } else { if (typeof r[0] !== "undefined") { p += parseInt(r[0].total_posts, 10); } else { if (typeof r[0] !== "undefined") {} } } }
                m.options.count[o] = p;
                m.options.total += p;
                m.renderer();
                m.rendererPerso(); }).error(function() { m.options.count[o] = 0;
                m.rendererPerso(); }); } else { m.renderer();
            m.options.count[o] = 0;
            m.rendererPerso(); } };
    k.prototype.rendererPerso = function() { var m = 0; for (e in this.options.count) { m++; } if (m === this.options.shareTotal) { this.options.render(this, this.options); } };
    k.prototype.renderer = function() { var n = this.options.total,
            m = this.options.template; if (this.options.shorterTotal === true) { n = this.shorterTotal(n); } if (m !== "") { m = m.replace("{total}", n);
    ...
    k.prototype.shorterTotal = function(m) { if (m >= 1000000) { m = (m / 1000000).toFixed(2) + "M"; } else { if (m >= 1000) { m = (m / 1000).toFixed(1) + "k"; } } return m; };
    k.prototype.openPopup = function(m) { a[m](this.options); if (this.options.enableTracking === true) { var n = { googlePlus: { site: "Google", action: "+1" }, facebook: { site: "facebook", action: "like" }, twitter: { site: "twitter", action: "tweet" }, digg: { site: "digg", action: "add" }, delicious: { site: "delicious", action: "add" }, stumbleupon: { site: "stumbleupon", action: "add" }, linkedin: { site: "linkedin", action: "share" }, pinterest: { site: "pinterest", action: "pin" } };
            _gaq.push(["_trackSocial", n[m].site, n[m].action]); } };
    k.prototype.simulateClick = function() { var m = g(this.element).html();
        g(this.element).html(m.replace(this.options.total, this.options.total + 1)); };
    k.prototype.update = function(m, n) { if (m !== "") { this.options.url = m; } if (n !== "") { this.options.text = n; } };
    g.fn[h] = function(n) { var m = arguments; if (n === b || typeof n === "object") { return this.each(function() { if (!g.data(this, "plugin_" + h)) { g.data(this, "plugin_" + h, new k(this, n)); } }); } else { if (typeof n === "string" && n[0] !== "_" && n !== "init") { return this.each(function() { var o = g.data(this, "plugin_" + h); if (o instanceof k && typeof o[n] === "function") { o[n].apply(o, Array.prototype.slice.call(m, 1)); } }); } } }; })(jQuery, window, document);
...
(function($, e, b) { var c = "hashchange",
    ...
    f = (function() { var j = {},
        ...
        function n() { var r = a(),
                q = o(m); if (r !== m) { l(m = r, q);
                $(e).trigger(c); } else { if (q !== m) { location.href = location.href.replace(/#.*/, "") + q; } }
            p = setTimeout(n, $.fn[c].delay); } return j; })(); })(jQuery, this); + function() { var scrollHandlerFn; var clickHandlerFn; var keyHandlerFn; var touchStartFn; var touchMoveFn; var disposeFn;

    function offset(element) { var offset = { top: 0, left: 0 }; if (!element.offsetParent) { return offset; }
        do { offset.left += element.offsetLeft;
            offset.top += element.offsetTop; } while (element = element.offsetParent); return offset; }

    function ZoomService() { this._activeZoom = this._initialScrollPosition = this._initialTouchPosition = this._touchMoveListener = null;
        this._document = document;
        this._window = window;
        this._body = document.body; }
    ZoomService.prototype.listen = function() { document.body.addEventListener("click", function(event) { if (event.target.getAttribute("data-action") === "zoom") { this._zoom(event); } }.bind(this)); };
    ZoomService.prototype._zoom = function(e) { var target = e.target; if (!target || target.tagName != "IMG") { return; } if (this._body.classList.contains("zoom-overlay-open")) { return; } if (e.metaKey || e.ctrlKey) { return window.open(e.target.getAttribute("data-original") || e.target.currentSrc || e.target.src, "_blank"); }
        ...
        e.stopPropagation(); };
    ZoomService.prototype._activeZoomClose = function(forceDispose) { if (!this._activeZoom) { return; } if (forceDispose) { this._activeZoom.dispose(); } else { this._activeZoom.close(); }
    ...
    ZoomService.prototype._scrollHandler = function(e) { if (this._initialScrollPosition === null) { this._initialScrollPosition = window.scrollY; } var deltaY = this._initialScrollPosition - window.scrollY; if (Math.abs(deltaY) >= 40) { this._activeZoomClose(); } };
    ...
    Zoom.prototype.zoomImage = function() { var img = document.createElement("img");
        img.onload = function() { this._fullHeight = Number(img.height);
    ...
    Zoom.prototype.close = function() { this._body.classList.remove("zoom-overlay-open");
        ...
        this._targetImageWrap.style.transform = ""; if (!"transition" in document.body.style) { return this.dispose(); }
    ...
    Zoom.prototype.dispose = function() { if (this._targetImageWrap && this._targetImageWrap.parentNode) { this._targetImage.classList.remove("zoom-img");
    ...
    $(function() { if (!window.isMobile || window.enableMobileZoom) {
            (new ZoomService).listen(); } }); }();
var _gsScope = "undefined" != typeof module && module.exports && "undefined" != typeof global ? global : this || window;
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push(function() { var t = document.documentElement,
        e = window,
        i = function(i, r) { var s = "x" === r ? "Width" : "Height",
                n = "scroll" + s,
                o = "client" + s,
                a = document.body; return i === e || i === t || i === a ? Math.max(t[n], a[n]) - (e["inner" + s] || t[o] || a[o]) : i[n] - i["offset" + s]; },
        r = _gsScope._gsDefine.plugin({ propName: "scrollTo", API: 2, version: "1.7.5", init: function(t, r, s) { return this._wdw = t === e, this._target = t, this._tween = s, "object" != typeof r && (r = { y: r }), this.vars = r, this._autoKill = r.autoKill !== !1, this.x = this.xPrev = this.getX(), this.y = this.yPrev = this.getY(), null != r.x ? (this._addTween(this, "x", this.x, "max" === r.x ? i(t, "x") : r.x, "scrollTo_x", !0), this._overwriteProps.push("scrollTo_x")) : this.skipX = !0, null != r.y ? (this._addTween(this, "y", this.y, "max" === r.y ? i(t, "y") : r.y, "scrollTo_y", !0), this._overwriteProps.push("scrollTo_y")) : this.skipY = !0, !0; }, set: function(t) { this._super.setRatio.call(this, t); var r = this._wdw || !this.skipX ? this.getX() : this.xPrev,
...
(function(t, e) { var i = t.GreenSockGlobals = t.GreenSockGlobals || t; if (!i.TweenLite) { var s, n, r, a, o, l = function(t) { var e, s = t.split("."),
            ...
            c = {},
            p = function(s, n, r, a) { this.sc = c[s] ? c[s].sc : [], c[s] = this, this.gsClass = null, this.func = r; var o = [];
                this.check = function(h) { for (var _, u, m, f, d = n.length, v = d; --d > -1;) {
        ...
        d.globals = i; var g = [0, 0, 1, 1],
            ...
            w = y.map = {},
            P = y.register = function(t, e, i, s) { for (var n, r, a, o, l = e.split(","), _ = l.length, u = (i || "easeIn,easeOut,easeInOut").split(","); --_ > -1;) { for (r = l[_], n = s ? v("easing." + r, null, !0) : h.easing[r] || {}, a = u.length; --a > -1;) { o = u[a], w[r + "." + o] = w[o + r] = n[o] = t.getRatio ? t : t[o] || new t; } } }; for (r = y.prototype, r._calcEnd = !1, r.getRatio = function(t) { if (this._func) { return this._params[0] = t, this._func.apply(null, this._params); } var e = this._type,
                    i = this._power,
                    s = 1 === e ? 1 - t : 2 === e ? t : 0.5 > t ? 2 * t : 2 * (1 - t); return 1 === i ? s *= s : 2 === i ? s *= s * s : 3 === i ? s *= s * s * s : 4 === i && (s *= s * s * s * s), 1 === e ? 1 - s : 2 === e ? s : 0.5 > t ? s / 2 : 1 - s / 2; }, s = ["Linear", "Quad", "Cubic", "Quart", "Quint,Strong"], n = s.length; --n > -1;) { r = s[n] + ",Power" + n, P(new y(null, null, 1, n), r, "easeOut", !0), P(new y(null, null, 2, n), r, "easeIn" + (0 === n ? ",easeNone" : "")), P(new y(null, null, 3, n), r, "easeInOut"); }
        ...
        r = b.prototype, r.addEventListener = function(t, e, i, s, n) { n = n || 0; var r, l, h = this._listeners[t],
                _ = 0; for (null == h && (this._listeners[t] = h = []), l = h.length; --l > -1;) { r = h[l], r.c === e && r.s === i ? h.splice(l, 1) : 0 === _ && n > r.pr && (_ = l + 1); }
            ...
            x = S(); for (s = ["ms", "moz", "webkit", "o"], n = s.length; --n > -1 && !k;) { k = t[s[n] + "RequestAnimationFrame"], A = t[s[n] + "CancelAnimationFrame"] || t[s[n] + "CancelRequestAnimationFrame"]; }
        v("Ticker", function(t, e) { var i, s, n, r, l, h = this,
                ...
                p = 33,
                d = "tick",
                v = function(t) { var e, a, o = S() - x;
                    o > c && (u += o - p), x += o, h.time = (x - u) / 1000, e = h.time - l, (!i || e > 0 || t === !0) && (h.frame++, l += e + (e >= r ? 0.004 : r - e), a = !0), t !== !0 && (n = s(v)), a && h.dispatchEvent(d); };
            b.call(h), h.time = h.frame = 0, h.tick = function() { v(!0); }, h.lagSmoothing = function(t, e) { c = t || 1 / _, p = Math.min(e, c, 0); }, h.sleep = function() { null != n && (f && A ? A(n) : clearTimeout(n), s = m, n = null, h === a && (o = !1)); }, h.wake = function() { null !== n ? h.sleep() : h.frame > 10 && (x = S() - c + 5), s = 0 === i ? m : f && k ? k : function(t) { return setTimeout(t, 0 | 1000 * (l - h.time) + 1); }, h === a && (o = !0), v(2); }, h.fps = function(t) { return arguments.length ? (i = t, r = 1 / (i || 60), l = this.time + r, h.wake(), void 0) : i; }, h.useRAF = function(t) { return arguments.length ? (h.sleep(), f = t, h.fps(i), void 0) : f; }, h.fps(t), setTimeout(function() { f && 5 > h.frame && h.useRAF(!1); }, 1500); }), r = h.Ticker.prototype = new h.events.EventDispatcher, r.constructor = h.Ticker; var R = v("core.Animation", function(t, e) { if (this.vars = e = e || {}, this._duration = this._totalDuration = t || 0, this._delay = Number(e.delay) || 0, this._timeScale = 1, this._active = e.immediateRender === !0, this.data = e.data, this._reversed = e.reversed === !0, B) { o || a.wake(); var i = this.vars.useFrames ? q : B;
                i.add(this, i._time), this.vars.paused && this.paused(!0); } });
        a = R.ticker = new h.Ticker, r = R.prototype, r._dirty = r._gc = r._initted = r._paused = !1, r._totalTime = r._time = 0, r._rawPrevTime = -1, r._next = r._last = r._onUpdate = r._timeline = r.timeline = null, r._paused = !1; var C = function() { o && S() - x > 2000 && a.wake(), setTimeout(C, 2000); };
        C(), r.play = function(t, e) { return null != t && this.seek(t, e), this.reversed(!1).paused(!1); }, r.pause = function(t, e) { return null != t && this.seek(t, e), this.paused(!0); }, r.resume = function(t, e) { return null != t && this.seek(t, e), this.paused(!1); }, r.seek = function(t, e) { return this.totalTime(Number(t), e !== !1); }, r.restart = function(t, e) { return this.reversed(!1).paused(!1).totalTime(t ? -this._delay : 0, e !== !1, !0); }, r.reverse = function(t, e) { return null != t && this.seek(t || this.totalDuration(), e), this.reversed(!0).paused(!1); }, r.render = function() {}, r.invalidate = function() { return this._time = this._totalTime = 0, this._initted = this._gc = !1, this._rawPrevTime = -1, (this._gc || !this.timeline) && this._enabled(!0), this; }, r.isActive = function() { var t, e = this._timeline,
                i = this._startTime; return !e || !this._gc && !this._paused && e.isActive() && (t = e.rawTime()) >= i && i + this.totalDuration() / this._timeScale > t; }, r._enabled = function(t, e) { return o || a.wake(), this._gc = !t, this._active = this.isActive(), e !== !0 && (t && !this.timeline ? this._timeline.add(this, this._startTime - this._delay) : !t && this.timeline && this._timeline._remove(this, !0)), !1; }, r._kill = function() { return this._enabled(!1, !1); }, r.kill = function(t, e) { return this._kill(t, e), this; }, r._uncache = function(t) { for (var e = t ? this : this.timeline; e;) { e._dirty = !0, e = e.timeline; } return this; }, r._swapSelfInParams = function(t) { for (var e = t.length, i = t.concat(); --e > -1;) { "{self}" === t[e] && (i[e] = this); } return i; }, r._callback = function(t) { var e = this.vars;
            e[t].apply(e[t + "Scope"] || e.callbackScope || this, e[t + "Params"] || T); }, r.eventCallback = function(t, e, i, s) { if ("on" === (t || "").substr(0, 2)) { var n = this.vars; if (1 === arguments.length) { return n[t]; }
        ...
        r = D.prototype = new R, r.constructor = D, r.kill()._gc = !1, r._first = r._last = r._recent = null, r._sortChildren = !1, r.add = r.insert = function(t, e) { var i, s; if (t._startTime = Number(e || 0) + t._delay, t._paused && this !== t._timeline && (t._pauseTime = t._startTime + (this.rawTime() - t._startTime) / t._timeScale), t.timeline && t.timeline._remove(t, !0), t.timeline = t._timeline = this, t._gc && t._enabled(!0, !0), i = this._last, this._sortChildren) { for (s = t._startTime; i && i._startTime > s;) { i = i._prev; } } return i ? (t._next = i._next, i._next = t) : (t._next = this._first, this._first = t), t._next ? t._next._prev = t : this._last = t, t._prev = i, this._recent = t, this._timeline && this._uncache(!0), this; }, r._remove = function(t, e) { return t.timeline === this && (e || t._enabled(!1, !0), t._prev ? t._prev._next = t._next : this._first === t && (this._first = t._next), t._next ? t._next._prev = t._prev : this._last === t && (this._last = t._prev), t._next = t._prev = t.timeline = null, t === this._recent && (this._recent = this._last), this._timeline && this._uncache(!0)), this; }, r.render = function(t, e, i) { var s, n = this._first; for (this._totalTime = this._time = this._rawPrevTime = t; n;) { s = n._next, (n._active || t >= n._startTime && !n._paused) && (n._reversed ? n.render((n._dirty ? n.totalDuration() : n._totalDuration) - (t - n._startTime) * n._timeScale, e, i) : n.render((t - n._startTime) * n._timeScale, e, i)), n = s; } }, r.rawTime = function() { return o || a.wake(), this._totalTime; }; var I = v("TweenLite", function(e, i, s) { if (R.call(this, i, s), this.render = I.prototype.render, null == e) { throw "Cannot tween a null target."; }
            ...
            O = function(t, e) { var i, s = {}; for (i in t) { G[i] || i in e && "transform" !== i && "x" !== i && "y" !== i && "width" !== i && "height" !== i && "className" !== i && "border" !== i || !(!F[i] || F[i] && F[i]._autoCSS) || (s[i] = t[i], delete t[i]); }
                t.css = s; };
        r = I.prototype = new R, r.constructor = I, r.kill()._gc = !1, r.ratio = 0, r._firstPT = r._targets = r._overwrittenProps = r._startAt = null, r._notifyPluginsOfEnabled = r._lazy = !1, I.version = "1.17.0", I.defaultEase = r._ease = new y(null, null, 1, 1), I.defaultOverwrite = "auto", I.ticker = a, I.autoSleep = 120, I.lagSmoothing = function(t, e) { a.lagSmoothing(t, e); }, I.selector = t.$ || t.jQuery || function(e) { var i = t.$ || t.jQuery; return i ? (I.selector = i, i(e)) : "undefined" == typeof document ? e : document.querySelectorAll ? document.querySelectorAll(e) : document.getElementById("#" === e.charAt(0) ? e.substr(1) : e); }; var z = [],
            ...
            M = 30,
            $ = L.lazyRender = function() { var t, e = z.length; for (N = {}; --e > -1;) { t = z[e], t && t._lazy !== !1 && (t.render(t._lazy[0], t._lazy[1], !0), t._lazy = !1); }
                z.length = 0; };
        B._startTime = a.time, q._startTime = a.frame, B._active = q._active = !0, setTimeout($, 1), R._updateRoot = I.render = function() { var t, e, i; if (z.length && $(), B.render((a.time - B._startTime) * B._timeScale, !1, !1), q.render((a.frame - q._startTime) * q._timeScale, !1, !1), z.length && $(), a.frame >= M) { M = a.frame + (parseInt(I.autoSleep, 10) || 120); for (i in U) { for (e = U[i].tweens, t = e.length; --t > -1;) { e[t]._gc && e.splice(t, 1); }
                    0 === e.length && delete U[i]; } if (i = B._first, (!i || i._paused) && I.autoSleep && !q._first && 1 === a._listeners.tick.length) { for (; i && i._paused;) { i = i._next; }
                    i || a.sleep(); } } }, a.addEventListener("tick", R._updateRoot); var K = function(t, e, i) { var s, n, r = t._gsTweenID; if (U[r || (t._gsTweenID = r = "t" + j++)] || (U[r] = { target: t, tweens: [] }), e && (s = U[r].tweens, s[n = s.length] = e, i)) { for (; --n > -1;) { s[n] === e && s.splice(n, 1); } } return U[r].tweens; },
            H = function(t, e, i, s) { var n, r, a = t.vars.onOverwrite; return a && (n = a(t, e, i, s)), a = I.onOverwrite, a && (r = a(t, e, i, s)), n !== !1 && r !== !1; },
            J = function(t, e, i, s, n) { var r, a, o, l; if (1 === s || s >= 4) { for (l = n.length, r = 0; l > r; r++) { if ((o = n[r]) !== e) { o._gc || o._kill(null, t, e) && (a = !0); } else { if (5 === s) { break; } } } return a; } var h, u = e._startTime + _,
                    m = [],
                    f = 0,
                    c = 0 === e._duration; for (r = n.length; --r > -1;) {
                    (o = n[r]) === e || o._gc || o._paused || (o._timeline !== e._timeline ? (h = h || V(e, 0, c), 0 === V(o, h, c) && (m[f++] = o)) : u >= o._startTime && o._startTime + o.totalDuration() / o._timeScale > u && ((c || !o._initted) && 2e-10 >= u - o._startTime || (m[f++] = o))); } for (r = f; --r > -1;) { if (o = m[r], 2 === s && o._kill(i, t, e) && (a = !0), 2 !== s || !o._firstPT && o._initted) { if (2 !== s && !H(o, e)) { continue; }
                        o._enabled(!1, !1) && (a = !0); } } return a; },
            V = function(t, e, i) { for (var s = t._timeline, n = s._timeScale, r = t._startTime; s._timeline;) { if (r += s._startTime, n *= s._timeScale, s._paused) { return -100; }
                    s = s._timeline; } return r /= n, r > e ? r - e : i && r === e || !t._initted && 2 * _ > r - e ? _ : (r += t.totalDuration() / t._timeScale / n) > e + _ ? 0 : r - e - _; };
        r._init = function() { var t, e, i, s, n, r = this.vars,
                a = this._overwrittenProps,
                o = this._duration,
                l = !!r.immediateRender,
                h = r.ease; if (r.startAt) { this._startAt && (this._startAt.render(-1, !0), this._startAt.kill()), n = {}; for (s in r.startAt) { n[s] = r.startAt[s]; } if (n.overwrite = !1, n.immediateRender = !0, n.lazy = l && r.lazy !== !1, n.startAt = n.delay = null, this._startAt = I.to(this.target, 0, n), l) { if (this._time > 0) { this._startAt = null; } else { if (0 !== o) { return; } } } } else { if (r.runBackwards && 0 !== o) { if (this._startAt) { this._startAt.render(-1, !0), this._startAt.kill(), this._startAt = null; } else { 0 !== this._time && (l = !1), i = {}; for (s in r) { G[s] && "autoCSS" !== s || (i[s] = r[s]); } if (i.overwrite = 0, i.data = "isFromStart", i.lazy = l && r.lazy !== !1, i.immediateRender = l, this._startAt = I.to(this.target, 0, i), l) { if (0 === this._time) { return; } } else { this._startAt._init(), this._startAt._enabled(!1), this.vars.immediateRender && (this._startAt = null); } } } } if (this._ease = h = h ? h instanceof y ? h : "function" == typeof h ? new y(h, r.easeParams) : w[h] || I.defaultEase : I.defaultEase, r.easeParams instanceof Array && h.config && (this._ease = h.config.apply(h, r.easeParams)), this._easeType = this._ease._type, this._easePower = this._ease._power, this._firstPT = null, this._targets) { for (t = this._targets.length; --t > -1;) { this._initProps(this._targets[t], this._propLookup[t] = {}, this._siblings[t], a ? a[t] : null) && (e = !0); } } else { e = this._initProps(this.target, this._propLookup, this._siblings, a); } if (e && I._onPluginEvent("_onInitAllProps", this), a && (this._firstPT || "function" != typeof this.target && this._enabled(!1, !1)), r.runBackwards) { for (i = this._firstPT; i;) { i.s += i.c, i.c = -i.c, i = i._next; } }
            this._onUpdate = r.onUpdate, this._initted = !0; }, r._initProps = function(e, i, s, n) { var r, a, o, l, h, _; if (null == e) { return !1; }
            N[e._gsTweenID] && $(), this.vars.css || e.style && e !== t && e.nodeType && F.css && this.vars.autoCSS !== !1 && O(this.vars, e); for (r in this.vars) { if (_ = this.vars[r], G[r]) { _ && (_ instanceof Array || _.push && f(_)) && -1 !== _.join("").indexOf("{self}") && (this.vars[r] = _ = this._swapSelfInParams(_, this)); } else { if (F[r] && (l = new F[r])._onInitTween(e, this.vars[r], this)) { for (this._firstPT = h = { _next: this._firstPT, t: l, p: "setRatio", s: 0, c: 1, f: !0, n: r, pg: !0, pr: l._priority }, a = l._overwriteProps.length; --a > -1;) { i[l._overwriteProps[a]] = this._firstPT; }(l._priority || l._onInitAllProps) && (o = !0), (l._onDisable || l._onEnable) && (this._notifyPluginsOfEnabled = !0); } else { this._firstPT = i[r] = h = { _next: this._firstPT, t: e, p: r, f: "function" == typeof e[r], n: r, pg: !1, pr: 0 }, h.s = h.f ? e[r.indexOf("set") || "function" != typeof e["get" + r.substr(3)] ? r : "get" + r.substr(3)]() : parseFloat(e[r]), h.c = "string" == typeof _ && "=" === _.charAt(1) ? parseInt(_.charAt(0) + "1", 10) * Number(_.substr(2)) : Number(_) - h.s || 0; } }
                h && h._next && (h._next._prev = h); } return n && this._kill(n, e) ? this._initProps(e, i, s, n) : this._overwrite > 1 && this._firstPT && s.length > 1 && J(e, this, i, this._overwrite, s) ? (this._kill(i, e), this._initProps(e, i, s, n)) : (this._firstPT && (this.vars.lazy !== !1 && this._duration || this.vars.lazy && !this._duration) && (N[e._gsTweenID] = !0), o); }, r.render = function(t, e, i) { var s, n, r, a, o = this._time,
                l = this._duration,
                h = this._rawPrevTime; if (t >= l) { this._totalTime = this._time = l, this.ratio = this._ease._calcEnd ? this._ease.getRatio(1) : 1, this._reversed || (s = !0, n = "onComplete", i = i || this._timeline.autoRemoveChildren), 0 === l && (this._initted || !this.vars.lazy || i) && (this._startTime === this._timeline._duration && (t = 0), (0 === t || 0 > h || h === _ && "isPause" !== this.data) && h !== t && (i = !0, h > _ && (n = "onReverseComplete")), this._rawPrevTime = a = !e || t || h === t ? t : _); } else { if (1e-7 > t) { this._totalTime = this._time = 0, this.ratio = this._ease._calcEnd ? this._ease.getRatio(0) : 0, (0 !== o || 0 === l && h > 0) && (n = "onReverseComplete", s = this._reversed), 0 > t && (this._active = !1, 0 === l && (this._initted || !this.vars.lazy || i) && (h >= 0 && (h !== _ || "isPause" !== this.data) && (i = !0), this._rawPrevTime = a = !e || t || h === t ? t : _)), this._initted || (i = !0); } else { if (this._totalTime = this._time = t, this._easeType) { var u = t / l,
                            m = this._easeType,
                            f = this._easePower;
                        (1 === m || 3 === m && u >= 0.5) && (u = 1 - u), 3 === m && (u *= 2), 1 === f ? u *= u : 2 === f ? u *= u * u : 3 === f ? u *= u * u * u : 4 === f && (u *= u * u * u * u), this.ratio = 1 === m ? 1 - u : 2 === m ? u : 0.5 > t / l ? u / 2 : 1 - u / 2; } else { this.ratio = this._ease.getRatio(t / l); } } } if (this._time !== o || i) { if (!this._initted) { if (this._init(), !this._initted || this._gc) { return; } if (!i && this._firstPT && (this.vars.lazy !== !1 && this._duration || this.vars.lazy && !this._duration)) { return this._time = this._totalTime = o, this._rawPrevTime = h, z.push(this), this._lazy = [t, e], void 0; }
                    this._time && !s ? this.ratio = this._ease.getRatio(this._time / l) : s && this._ease._calcEnd && (this.ratio = this._ease.getRatio(0 === this._time ? 0 : 1)); } for (this._lazy !== !1 && (this._lazy = !1), this._active || !this._paused && this._time !== o && t >= 0 && (this._active = !0), 0 === o && (this._startAt && (t >= 0 ? this._startAt.render(t, e, i) : n || (n = "_dummyGS")), this.vars.onStart && (0 !== this._time || 0 === l) && (e || this._callback("onStart"))), r = this._firstPT; r;) { r.f ? r.t[r.p](r.c * this.ratio + r.s) : r.t[r.p] = r.c * this.ratio + r.s, r = r._next; }
                this._onUpdate && (0 > t && this._startAt && t !== -0.0001 && this._startAt.render(t, e, i), e || (this._time !== o || s) && this._callback("onUpdate")), n && (!this._gc || i) && (0 > t && this._startAt && !this._onUpdate && t !== -0.0001 && this._startAt.render(t, e, i), s && (this._timeline.autoRemoveChildren && this._enabled(!1, !1), this._active = !1), !e && this.vars[n] && this._callback(n), 0 === l && this._rawPrevTime === _ && a !== _ && (this._rawPrevTime = 0)); } }, r._kill = function(t, e, i) { if ("all" === t && (t = null), null == t && (null == e || e === this.target)) { return this._lazy = !1, this._enabled(!1, !1); }
            e = "string" != typeof e ? e || this._targets || this.target : I.selector(e) || e; var s, n, r, a, o, l, h, _, u, m = i && this._time && i._startTime === this._startTime && this._timeline === i._timeline; if ((f(e) || E(e)) && "number" != typeof e[0]) { for (s = e.length; --s > -1;) { this._kill(t, e[s], i) && (l = !0); } } else { if (this._targets) { for (s = this._targets.length; --s > -1;) { if (e === this._targets[s]) { o = this._propLookup[s] || {}, this._overwrittenProps = this._overwrittenProps || [], n = this._overwrittenProps[s] = t ? this._overwrittenProps[s] || {} : "all"; break; } } } else { if (e !== this.target) { return !1; }
                    o = this._propLookup, n = this._overwrittenProps = t ? this._overwrittenProps || {} : "all"; } if (o) { if (h = t || o, _ = t !== n && "all" !== n && t !== o && ("object" != typeof t || !t._tempKill), i && (I.onOverwrite || this.vars.onOverwrite)) { for (r in h) { o[r] && (u || (u = []), u.push(r)); } if ((u || !t) && !H(this, i, e, u)) { return !1; } } for (r in h) {
                        (a = o[r]) && (m && (a.f ? a.t[a.p](a.s) : a.t[a.p] = a.s, l = !0), a.pg && a.t._kill(h) && (l = !0), a.pg && 0 !== a.t._overwriteProps.length || (a._prev ? a._prev._next = a._next : a === this._firstPT && (this._firstPT = a._next), a._next && (a._next._prev = a._prev), a._next = a._prev = null), delete o[r]), _ && (n[r] = 1); }!this._firstPT && this._initted && this._enabled(!1, !1); } } return l; }, r.invalidate = function() { return this._notifyPluginsOfEnabled && I._onPluginEvent("_onDisable", this), this._firstPT = this._overwrittenProps = this._startAt = this._onUpdate = null, this._notifyPluginsOfEnabled = this._active = this._lazy = !1, this._propLookup = this._targets ? {} : [], R.prototype.invalidate.call(this), this.vars.immediateRender && (this._time = -_, this.render(-this._delay)), this; }, r._enabled = function(t, e) { if (o || a.wake(), t && this._gc) { var i, s = this._targets; if (s) { for (i = s.length; --i > -1;) { this._siblings[i] = K(s[i], this, !0); } } else { this._siblings = K(this.target, this, !0); } } return R.prototype._enabled.call(this, t, e), this._notifyPluginsOfEnabled && this._firstPT ? I._onPluginEvent(t ? "_onEnable" : "_onDisable", this) : !1; }, I.to = function(t, e, i) { return new I(t, e, i); }, I.from = function(t, e, i) { return i.runBackwards = !0, i.immediateRender = 0 != i.immediateRender, new I(t, e, i); }, I.fromTo = function(t, e, i, s) { return s.startAt = i, s.immediateRender = 0 != s.immediateRender && 0 != i.immediateRender, new I(t, e, s); }, I.delayedCall = function(t, e, i, s, n) { return new I(e, 0, { delay: t, onComplete: e, onCompleteParams: i, callbackScope: s, onReverseComplete: e, onReverseCompleteParams: i, immediateRender: !1, lazy: !1, useFrames: n, overwrite: 0 }); }, I.set = function(t, e) { return new I(t, 0, e); }, I.getTweensOf = function(t, e) { if (null == t) { return []; }
            t = "string" != typeof t ? t : I.selector(t) || t; var i, s, n, r; if ((f(t) || E(t)) && "number" != typeof t[0]) { for (i = t.length, s = []; --i > -1;) { s = s.concat(I.getTweensOf(t[i], e)); } for (i = s.length; --i > -1;) { for (r = s[i], n = i; --n > -1;) { r === s[n] && s.splice(i, 1); } } } else { for (s = K(t).concat(), i = s.length; --i > -1;) {
                    (s[i]._gc || e && !s[i].isActive()) && s.splice(i, 1); } } return s; }, I.killTweensOf = I.killDelayedCallsTo = function(t, e, i) { "object" == typeof e && (i = e, e = !1); for (var s = I.getTweensOf(t, e), n = s.length; --n > -1;) { s[n]._kill(i, t); } }; var W = v("plugins.TweenPlugin", function(t, e) { this._overwriteProps = (t || "").split(","), this._propName = this._overwriteProps[0], this._priority = e || 0, this._super = W.prototype; }, !0); if (r = W.prototype, W.version = "1.10.1", W.API = 2, r._firstPT = null, r._addTween = function(t, e, i, s, n, r) { var a, o; return null != s && (a = "number" == typeof s || "=" !== s.charAt(1) ? Number(s) - Number(i) : parseInt(s.charAt(0) + "1", 10) * Number(s.substr(2))) ? (this._firstPT = o = { _next: this._firstPT, t: t, p: e, s: i, c: a, f: "function" == typeof t[e], n: n || e, r: r }, o._next && (o._next._prev = o), o) : void 0; }, r.setRatio = function(t) { for (var e, i = this._firstPT, s = 0.000001; i;) { e = i.c * t + i.s, i.r ? e = Math.round(e) : s > e && e > -s && (e = 0), i.f ? i.t[i.p](e) : i.t[i.p] = e, i = i._next; } }, r._kill = function(t) { var e, i = this._overwriteProps,
                    s = this._firstPT; if (null != t[this._propName]) { this._overwriteProps = []; } else { for (e = i.length; --e > -1;) { null != t[i[e]] && i.splice(e, 1); } } for (; s;) { null != t[s.n] && (s._next && (s._next._prev = s._prev), s._prev ? (s._prev._next = s._next, s._prev = null) : this._firstPT === s && (this._firstPT = s._next)), s = s._next; } return !1; }, r._roundProps = function(t, e) { for (var i = this._firstPT; i;) {
                    (t[this._propName] || null != i.n && t[i.n.split(this._propName + "_").join("")]) && (i.r = e), i = i._next; } }, I._onPluginEvent = function(t, e) { var i, s, n, r, a, o = e._firstPT; if ("_onInitAllProps" === t) { for (; o;) { for (a = o._next, s = n; s && s.pr > o.pr;) { s = s._next; }(o._prev = s ? s._prev : r) ? o._prev._next = o: n = o, (o._next = s) ? s._prev = o : r = o, o = a; }
                    o = e._firstPT = n; } for (; o;) { o.pg && "function" == typeof o.t[t] && o.t[t]() && (i = !0), o = o._next; } return i; }, W.activate = function(t) { for (var e = t.length; --e > -1;) { t[e].API === W.API && (F[(new t[e])._propName] = t[e]); } return !0; }, d.plugin = function(t) { if (!(t && t.propName && t.init && t.API)) { throw "illegal plugin definition."; } var e, i = t.propName,
                    s = t.priority || 0,
                    n = t.overwriteProps,
                ...
                o.constructor = a, a.API = t.API; for (e in r) { "function" == typeof t[e] && (o[r[e]] = t[e]); } return a.version = t.version, W.activate([a]), a; }, s = t._gsQueue) { for (n = 0; s.length > n; n++) { s[n](); } for (r in c) { c[r].func || t.console.log("GSAP encountered missing dependency: com.greensock." + r); } }
        o = !1; } })("undefined" != typeof module && module.exports && "undefined" != typeof global ? global : this || window, "TweenLite");
(function(document, window) { document.addEventListener("DOMContentLoaded", function() { var baseUrl = window.location.href.replace(window.location.hash, "");
        [].slice.call(document.querySelectorAll("use[*|href]")).filter(function(element) { return (element.getAttribute("xlink:href").indexOf("#") === 0); }).forEach(function(element) { element.setAttribute("xlink:href", baseUrl.replace("#", "") + element.getAttribute("xlink:href")); }); }, false); }(document, window));
...
function(t, e) { "function" == typeof define && define.amd ? define("fizzy-ui-utils/utils", ["desandro-matches-selector/matches-selector"], function(i) { return e(t, i); }) : "object" == typeof module && module.exports ? module.exports = e(t, require("desandro-matches-selector")) : t.fizzyUIUtils = e(t, t.matchesSelector); }(window, function(t, e) { var i = {};
    i.extend = function(t, e) { for (var i in e) { t[i] = e[i]; } return t; }, i.modulo = function(t, e) { return (t % e + e) % e; }, i.makeArray = function(t) { var e = []; if (Array.isArray(t)) { e = t; } else { if (t && "object" == typeof t && "number" == typeof t.length) { for (var i = 0; i < t.length; i++) { e.push(t[i]); } } else { e.push(t); } } return e; }, i.removeFrom = function(t, e) { var i = t.indexOf(e); - 1 != i && t.splice(i, 1); }, i.getParent = function(t, i) { for (; t.parentNode && t != document.body;) { if (t = t.parentNode, e(t, i)) { return t; } } }, i.getQueryElement = function(t) { return "string" == typeof t ? document.querySelector(t) : t; }, i.handleEvent = function(t) { var e = "on" + t.type;
...
function(t, e) { "function" == typeof define && define.amd ? define("outlayer/outlayer", ["ev-emitter/ev-emitter", "get-size/get-size", "fizzy-ui-utils/utils", "./item"], function(i, n, o, r) { return e(t, i, n, o, r); }) : "object" == typeof module && module.exports ? module.exports = e(t, require("ev-emitter"), require("get-size"), require("fizzy-ui-utils"), require("./item")) : t.Outlayer = e(t, t.EvEmitter, t.getSize, t.fizzyUIUtils, t.Outlayer.Item); }(window, function(t, e, i, n, o) {
    ...
    n.extend(f, e.prototype), f.option = function(t) { n.extend(this.options, t); }, f._getOption = function(t) { var e = this.constructor.compatOptions[t]; return e && void 0 !== this.options[e] ? this.options[e] : this.options[t]; }, r.compatOptions = { initLayout: "isInitLayout", horizontal: "isHorizontal", layoutInstant: "isLayoutInstant", originLeft: "isOriginLeft", originTop: "isOriginTop", resize: "isResizeBound", resizeContainer: "isResizingContainer" }, f._create = function() { this.reloadItems(), this.stamps = [], this.stamp(this.options.stamp), n.extend(this.element.style, this.options.containerStyle); var t = this._getOption("resize");
        ...
        e.forEach(function(e) { e.once(t, n); }); }, f.dispatchEvent = function(t, e, i) { var n = e ? [e].concat(i) : i; if (this.emitEvent(t, n), u) { if (this.$element = this.$element || u(this.element), e) { var o = u.Event(e);
...
function(e, t) { "function" == typeof define && define.amd ? define(["ev-emitter/ev-emitter"], function(i) { return t(e, i); }) : "object" == typeof module && module.exports ? module.exports = t(e, require("ev-emitter")) : e.imagesLoaded = t(e, e.EvEmitter); }("undefined" != typeof window ? window : this, function(e, t) {
    ...
    o.prototype = Object.create(t.prototype), o.prototype.options = {}, o.prototype.getImages = function() { this.images = [], this.elements.forEach(this.addElementImages, this); }, o.prototype.addElementImages = function(e) { "IMG" == e.nodeName && this.addImage(e), this.options.background === !0 && this.addElementBackgroundImages(e); var t = e.nodeType; if (t && u[t]) { for (var i = e.querySelectorAll("img"), n = 0; n < i.length; n++) { var o = i[n];
                this.addImage(o); } if ("string" == typeof this.options.background) { var r = e.querySelectorAll(this.options.background); for (n = 0; n < r.length; n++) { var s = r[n];
                    this.addElementBackgroundImages(s); } } } }; var u = { 1: !0, 9: !0, 11: !0 }; return o.prototype.addElementBackgroundImages = function(e) { var t = getComputedStyle(e); if (t) { for (var i = /url\((['"])?(.*?)\1\)/gi, n = i.exec(t.backgroundImage); null !== n;) { var o = n && n[2];
...
! function(e) { "function" == typeof define && define.amd ? define(["jquery"], e) : e("undefined" != typeof jQuery ? jQuery : window.Zepto); }(function(e) {
    ...
    function r(t) { var r = t.target,
            ...
            r = n[0]; } var i = this; if (i.clk = r, "image" == r.type) { if (void 0 !== t.offsetX) { i.clk_x = t.offsetX, i.clk_y = t.offsetY; } else { if ("function" == typeof e.fn.offset) { var o = a.offset();
    ...
    e.fn.attr2 = function() { if (!i) { return this.attr.apply(this, arguments); } var e = this.prop.apply(this, arguments); return e && e.jquery || "string" == typeof e ? e : this.attr.apply(this, arguments); }, e.fn.ajaxSubmit = function(t) {
        ...
        function s(r) {
            ...
            function o() {
                ...
                w.setAttribute("target", p), (!u || /post/i.test(u)) && w.setAttribute("method", "POST"), i != m.url && w.setAttribute("action", m.url), m.skipEncodingOverride || u && !/post/i.test(u) || f.attr({ encoding: "multipart/form-data", enctype: "multipart/form-data" }), m.timeout && (j = setTimeout(function() { T = !0, s(D); }, m.timeout)); var l = []; try { if (m.extraData) { for (var d in m.extraData) { m.extraData.hasOwnProperty(d) && l.push(e.isPlainObject(m.extraData[d]) && m.extraData[d].hasOwnProperty("name") && m.extraData[d].hasOwnProperty("value") ? e('<input type="hidden" name="' + m.extraData[d].name + '">').val(m.extraData[d].value).appendTo(w)[0] : e('<input type="hidden" name="' + d + '">').val(m.extraData[d]).appendTo(w)[0]); } }
            ...
            function s(t) { if (!x.aborted && !F) { if (M = n(g), M || (a("cannot access response document"), t = k), t === D && x) { return x.abort("timeout"), void S.reject(x, "timeout"); } if (t == k && x) { return x.abort("server abort"), void S.reject(x, "error", "server abort"); } if (M && M.location.href != m.iframeSrc || T) { g.detachEvent ? g.detachEvent("onload", s) : g.removeEventListener("load", s, !1); var r, i = "success"; try { if (T) { throw "timeout"; } var o = "xml" == m.dataType || M.XMLDocument || e.isXMLDoc(M); if (a("isXml=" + o), !o && window.opera && (null === M.body || !M.body.innerHTML) && --O) { return a("requeing onLoad callback, DOM not available"), void setTimeout(s, 250); } var u = M.body ? M.body : M.documentElement;
                            x.responseText = u ? u.innerHTML : null, x.responseXML = M.XMLDocument ? M.XMLDocument : M, o && (m.dataType = "xml"), x.getResponseHeader = function(e) { var t = { "content-type": m.dataType }; return t[e.toLowerCase()]; }, u && (x.status = Number(u.getAttribute("status")) || x.status, x.statusText = u.getAttribute("statusText") || x.statusText); var c = (m.dataType || "").toLowerCase(),
                                l = /(json|script|text)/.test(c); if (l || m.textarea) { var f = M.getElementsByTagName("textarea")[0]; if (f) { x.responseText = f.value, x.status = Number(f.getAttribute("status")) || x.status, x.statusText = f.getAttribute("statusText") || x.statusText; } else { if (l) { var p = M.getElementsByTagName("pre")[0],
        ...
        t.iframe !== !1 && (t.iframe || L) ? t.closeKeepAlive ? e.get(t.closeKeepAlive, function() { A = s(v); }) : A = s(v) : A = (w || D) && k ? o(v) : e.ajax(t), f.removeData("jqxhr").data("jqxhr", A); for (var E = 0; E < h.length; E++) { h[E] = null; } return this.trigger("form-submit-notify", [this, t]), this; }, e.fn.ajaxForm = function(n) { if (n = n || {}, n.delegation = n.delegation && e.isFunction(e.fn.on), !n.delegation && 0 === this.length) { var i = { s: this.selector, c: this.context }; return !e.isReady && i.s ? (a("DOM not ready, queuing ajaxForm"), e(function() { e(i.s, i.c).ajaxForm(n); }), this) : (a("terminating; zero elements found by selector" + (e.isReady ? "" : " (DOM not ready)")), this); } return n.delegation ? (e(document).off("submit.form-plugin", this.selector, t).off("click.form-plugin", this.selector, r).on("submit.form-plugin", this.selector, n, t).on("click.form-plugin", this.selector, n, r), this) : this.ajaxFormUnbind().bind("submit.form-plugin", n, t).bind("click.form-plugin", n, r); }, e.fn.ajaxFormUnbind = function() { return this.unbind("submit.form-plugin click.form-plugin"); }, e.fn.formToArray = function(t, r) { var a = []; if (0 === this.length) { return a; } var i, o = this[0],
            ...
            ("function" == typeof this.reset || "object" == typeof this.reset && !this.reset.nodeType) && this.reset(); }); }, e.fn.enable = function(e) { return void 0 === e && (e = !0), this.each(function() { this.disabled = !e; }); }, e.fn.selected = function(t) { return void 0 === t && (t = !0), this.each(function() { var r = this.type; if ("checkbox" == r || "radio" == r) { this.checked = t; } else { if ("option" == this.tagName.toLowerCase()) { var a = e(this).parent("select");
                    t && a[0] && "select-one" == a[0].type && a.find("option").selected(!1), this.selected = t; } } }); }, e.fn.ajaxSubmit.debug = !1; });
$.cookie = function(key, value, options) { if (arguments.length > 1 && String(value) !== "[object Object]") { options = $.extend({}, options); if (value === null || value === undefined) { options.expires = -1; } if (typeof options.expires === "number") { var days = options.expires,
                t = options.expires = new Date();
            t.setDate(t.getDate() + days); }
        value = String(value); return (document.cookie = [encodeURIComponent(key), "=", options.raw ? value : encodeURIComponent(value), options.expires ? "; expires=" + options.expires.toUTCString() : "", options.path ? "; path=" + options.path : "", options.domain ? "; domain=" + options.domain : "", options.secure ? "; secure" : ""].join("")); }
    options = value || {}; var result, decode = options.raw ? function(s) { return s; } : decodeURIComponent; return (result = new RegExp("(?:^|; )" + encodeURIComponent(key) + "=([^;]*)").exec(document.cookie)) ? decode(result[1]) : null; };
! function(e, n, t) { "undefined" != typeof module && module.exports ? module.exports = t(n, e) : "function" == typeof define && define.amd ? define("detect-zoom", function() { return t(n, e); }) : e[n] = t(n, e); }(window, "detectZoom", function() { var e = function() { return window.devicePixelRatio || 1; },
        n = function() { return { zoom: 1, devicePxPerCssPx: 1 }; },
        t = function() { var n = Math.round(screen.deviceXDPI / screen.logicalXDPI * 100) / 100; return { zoom: n, devicePxPerCssPx: n * e() }; },
        i = function() { var n = Math.round(document.documentElement.offsetHeight / window.innerHeight * 100) / 100; return { zoom: n, devicePxPerCssPx: n * e() }; },
        o = function() { var n = Math.round(window.outerWidth / window.innerWidth * 100) / 100; return { zoom: n, devicePxPerCssPx: n * e() }; },
        r = function() { var n = Math.round(document.documentElement.clientWidth / window.innerWidth * 100) / 100; return { zoom: n, devicePxPerCssPx: n * e() }; },
        d = function() { var n = 90 == Math.abs(window.orientation) ? screen.height : screen.width,
                t = n / window.innerWidth; return { zoom: t, devicePxPerCssPx: t * e() }; },
        u = function() { var n = function(e) { return e.replace(/;/g, " !important;"); },
                t = document.createElement("div");
            t.innerHTML = "1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>9<br>0", t.setAttribute("style", n("font: 100px/1em sans-serif; -webkit-text-size-adjust: none; text-size-adjust: none; height: auto; width: 1em; padding: 0; overflow: visible;")); var i = document.createElement("div");
            i.setAttribute("style", n("width:0; height:0; overflow:hidden; visibility:hidden; position: absolute;")), i.appendChild(t), document.body.appendChild(i); var o = 1000 / t.clientHeight; return o = Math.round(100 * o) / 100, document.body.removeChild(i), { zoom: o, devicePxPerCssPx: o * e() }; },
        a = function() { var e = m("min--moz-device-pixel-ratio", "", 0, 10, 20, 0.0001); return e = Math.round(100 * e) / 100, { zoom: e, devicePxPerCssPx: e }; },
        c = function() { return { zoom: a().zoom, devicePxPerCssPx: e() }; },
        s = function() { var n = window.top.outerWidth / window.top.innerWidth; return n = Math.round(100 * n) / 100, { zoom: n, devicePxPerCssPx: n * e() }; },
        m = function(e, n, t, i, o, r) {
            function d(t, i, o) { var a = (t + i) / 2; if (0 >= o || r > i - t) { return a; } var c = "(" + e + ":" + a + n + ")"; return u(c).matches ? d(a, i, o - 1) : d(t, a, o - 1); } var u, a, c, s;
            window.matchMedia ? u = window.matchMedia : (a = document.getElementsByTagName("head")[0], c = document.createElement("style"), a.appendChild(c), s = document.createElement("div"), s.className = "mediaQueryBinarySearch", s.style.display = "none", document.body.appendChild(s), u = function(e) { c.sheet.insertRule("@media " + e + "{.mediaQueryBinarySearch {text-decoration: underline} }", 0); var n = "underline" == getComputedStyle(s, null).textDecoration; return c.sheet.deleteRule(0), { matches: n }; }); var m = d(t, i, o); return s && (a.removeChild(c), document.body.removeChild(s)), m; },
        l = function() { var e = n; return isNaN(screen.logicalXDPI) || isNaN(screen.systemXDPI) ? window.navigator.msMaxTouchPoints ? e = i : window.chrome && !(window.opera || navigator.userAgent.indexOf(" Opera") >= 0) ? e = o : Object.prototype.toString.call(window.HTMLElement).indexOf("Constructor") > 0 ? e = r : "orientation" in window && "webkitRequestAnimationFrame" in window ? e = d : "webkitRequestAnimationFrame" in window ? e = u : navigator.userAgent.indexOf("Opera") >= 0 ? e = s : window.devicePixelRatio ? e = c : a().zoom > 0.001 && (e = a) : e = t, e; }(); return { zoom: function() { return l().zoom; }, device: function() { return l().devicePxPerCssPx; } }; });
window.inAction = 1;
window.allowSlide = 1;
...
var $html = $("html");
$(window).on("load", function() { window.loaded = 1; });
$(document).ready(function() { var $body = $("body");
    setTimeout(function() { $(window).trigger("ready"); }, 1);
    $body.hide().show(0);
    window.isScroll = $body.hasClass("scroll");
    window.isSimplifiedMobile = $body.hasClass("simplifiedMobile"); if (window.isScroll || window.isSimplifiedMobile && window.isMobile) { $html.addClass("scrollable"); }
    $html.addClass("page-ready"); if ($body.hasClass("fast")) { window.slideSpeed = 700;
        window.cleanupDelay = 1200;
        window.effectSpeed = 800;
        window.scrollSpeed = 0.35;
        window.effectOffset = 400; } else { if ($body.hasClass("slow")) { window.slideSpeed = 1400;
    ...
    window.stage = 1;
    window.stages = $(".slide").length; if ($body.hasClass("horizontal")) { window.horizontalMode = 1; } if ($body.hasClass("noPreload")) { window.preload = 0; } if ($body.hasClass("animated")) { window.isAnimated = "auto"; } else { if ($body.hasClass("animateOnEvent")) { window.isAnimated = "animateOnEvent"; if (window.isMobile) { window.isAnimated = "auto";
                $body.removeClass("animateOnEvent").addClass("animated"); } } } if (window.isSimplifiedMobile && window.isMobile) { window.isAnimated = false;
        $body.removeClass("animated animateOnEvent");
        $("[class*='ae-']").addClass("done"); } if (!window.isAnimated) { window.cleanupDelay = 0; } if ($body.hasClass("smoothScroll") && !window.isMobile) { window.smoothScroll = 1; }

    function updateHash() { var hashLink = window.location.href.split("#")[1]; if (hashLink) { if ($('.slide[data-name="' + hashLink + '"]').length > 0) { var requestedElement = $('.slide[data-name="' + hashLink + '"]'); if ((window.isMobile && window.isSimplifiedMobile) || window.isScroll) { if (requestedElement.length) { if (!window.preload || window.loaded) { $("html,body").stop().clearQueue().animate({ scrollTop: requestedElement.position().top }, window.effectSpeed); } else { $(window).on("load", function() { $("html,body").stop().clearQueue().animate({ scrollTop: requestedElement.position().top }, window.effectSpeed); }); } } } else { window.stage = $(".slide").index(requestedElement) + 1;
                    showSlide(window.stage); } } } }
    updateHash();
    $(window).on("popstate", function(e) { setTimeout(function() { updateHash(); }, 100);
        e.preventDefault(); }); if (window.preload) { var imgs = [];
        $("*").each(function() { if ($(this).css("background-image") !== "none") { imgs.push($(this).css("background-image").replace(/.*\s?url\([\'\"]?/, "").replace(/[\'\"]?\).*/, "")); } else { if ($(this).is("img")) { imgs.push($(this).attr("src")); } } });
        window.images = imgs.length;
        window.progressBar = $(".progress-bar");
        $.cacheImage(imgs, { complete: function() { window.loadingProgress++;
                updateProgressBar(); } });

        function updateProgressBar() { var progress = window.loadingProgress / window.images;
            window.progressBar.css("width", progress * 100 + "%"); if (window.loadingProgress == window.images) { window.progressBar.addClass("loaded"); } }
        updateProgressBar(); }
    showSlide(window.stage);
    $(".grid.masonry").masonry({ itemSelector: "li", transitionDuration: "0.1s" });
    $(".grid.masonry").imagesLoaded().progress(function() { $(".grid.masonry").masonry("layout"); }); if (!window.preload || !window.images || window.loaded) { runTheCode(); } if (!window.loaded) { $(window).on("load", function() { runTheCode(); }); }

    function runTheCode() { $html.addClass("page-loaded");
        window.inAction = 0;
        window.blockScroll = 0;
        window.loaded = 1;
        setTimeout(function() { if (window.isScroll) { updateScroll();
                updateNavigation(); } if (window.isMobile && window.isSimplifiedMobile) { $(".slide").addClass("selected animate active");
                updateScroll();
                updateNavigation(); } else { showSlide(window.stage); } }, 500); }

    function showSlide(requested) { requested = parseInt(requested); if (window.isMobile && window.isSimplifiedMobile || window.isScroll) { return; }
        updateNavigation(); var newSlide = $(".slide").eq(requested - 1),
        ...
        hideSidebar();
        window.allowSlide = 1;
        $body.removeClass("sidebarShown lastSlide firstSlide hidePanel-top hidePanel-bottom"); if (window.setStageClasses != 0) { if (window.stage === 1) { $body.addClass("firstSlide"); } if ((window.stages === window.stage) && (window.stages !== 1)) { $body.addClass("lastSlide"); }
            $body.removeClassByPrefix("stage-").addClass("stage-" + window.stage); } if (newSlide.hasClass("whiteSlide")) { $body.addClass("whiteSlide"); } else { $body.removeClass("whiteSlide"); } if (currenSlideIndex !== requested && window.setStageClasses != 0) { currenSlide.removeClass("selected").addClass("active");
            ...
            $(window).trigger("slideChange", [parseInt(requested), newSlide]); } if (window.setHashLink) { if (newSlide.attr("data-name") || newSlide.attr("id")) { window.location.hash = (newSlide.attr("data-name")) ? newSlide.attr("data-name") : newSlide.attr("id"); } else { if ((window.location.toString().indexOf("#") > 0) && (location.protocol !== "file:") && (location.href.split("#")[0])) { if (history.pushState) { window.history.pushState("", "", location.href.split("#")[0]); } else { window.location.hash = ""; } } } }
        newSlide.find(".content, .container").scrollTop(0); if (window.loaded) { window.blockScroll = 1;
            setTimeout(function() { if (currenSlideIndex !== requested) { currenSlide.removeClass("active animate"); }
                window.blockScroll = 0; }, window.effectSpeed); if (window.effectOffset > window.slideSpeed) { window.effectOffset = window.slideSpeed; }
            setTimeout(function() { newSlide.addClass("animate"); }, window.slideSpeed - window.effectOffset);
            $(".done").removeClass("done");
            clearTimeout(window.clearElementAnimation);
            window.clearElementAnimation = setTimeout(function() { $(".slide.selected [class*='ae-']").addClass("done"); }, window.slideSpeed + window.effectSpeed + window.cleanupDelay); } }
    $(".animated").on("click", "[class*='ae-']:not('.done')", function() { $(this).addClass("done"); });
    window.changeSlide = function(n) { if (n === "increase") { if ((window.stage + 1) >= window.stages) { n = window.stages; } else { n = window.stage + 1; } } else { if (n === "decrease") { if ((window.stage - 1) < 1) { n = 1; } else { n = window.stage - 1; } } } if (window.isMobile && window.isSimplifiedMobile || window.isScroll) { window.stage = n; var requestedElement = $(".slide:eq(" + (window.stage - 1) + ")"),
                finalPosition = $(requestedElement).offset().top;
            $("html,body").stop().clearQueue().animate({ scrollTop: finalPosition }, 1000); } else { if ((n !== window.stage) && (n <= window.stages)) { if (window.inAction !== 1) { window.inAction = 1;
                    window.stage = n; var delay = 0; if ($(".zoom-overlay-open").length > 0) { unzoomImage();
                        delay = 550; }
                    setTimeout(function() { showSlide(window.stage);
                        setTimeout(function() { window.inAction = 0; }, window.slideSpeed); }, delay); } } } };
    $(".nextSlide").on("click", function() { window.changeSlide("increase"); });
    $(".prevSlide").on("click", function() { window.changeSlide("decrease"); });
    $(".toFirstSlide").on("click", function() { window.changeSlide(1); if (history.pushState) { window.history.pushState("", "", location.href.split("#")[0]); } else { window.location.hash = ""; }
        hideSidebar(); });
    $(".toLastSlide").on("click", function() { window.changeSlide(window.stages); if (history.pushState) { window.history.pushState("", "", location.href.split("#")[0]); } else { window.location.hash = ""; }
        hideSidebar(); });
    $('[class*="toSlide-"]').on("click", function() { var num = parseInt($(this).attr("class").split("toSlide-")[1].split(" ")[0]);
        window.changeSlide(num);
        hideSidebar(); });

    function unzoomImage(type) { if ($(".zoom-overlay-open").length > 0) { $(".zoom-img").click(); if (type) { $(".zoom-img-wrap, .zoom-overlay").remove(); } } }
    $(window).on("resize load ready", function() { $('[data-action="zoom"]').removeAttr("style"); if ($(".zoom-overlay").length > 0) { unzoomImage("fast"); }
        window.windowHeight = $(window).height();
        window.windowWidth = $(window).width();
        window.documentHeight = $(document).height(); }); var eventCount = 0,
        eventCountStart;
    $("html,body").on("DOMMouseScroll mousewheel scroll touchmove", function(event) { var $currentSection = $(".slide.selected .content"),
            ...
            minScrollToSlide = (window.isFirefox && window.isWindows) ? 200 : window.minScrollToSlide; if (!scrollsize) { return; } if (window.isScroll && ((!window.sidebarShown) && (!window.popupShown) && (!window.blockScroll))) { if (window.smoothScroll && !window.isMobile) { event.preventDefault(); if (energy > 1500) { energy = 1500; } if (energy < -1000) { energy = -1500; } var scrollObject = $(window),
                    scrollTop = scrollObject.scrollTop(),
                    finalScroll = scrollTop - energy;
                TweenLite.to(scrollObject, window.scrollSpeed, { scrollTo: { y: finalScroll, autoKill: false }, ease: Power4.easeOut, overwrite: "all" }); } else { if (!window.isWindows) { $currentSection.scrollTop(curSecScrolltop - energy); } } } if (!window.isScroll && !(window.isMobile && window.isSimplifiedMobile)) { if ((currentSectionHeight > window.windowHeight)) { if (((scrollDirection === "up") && ($currentSection.scrollTop() === 0)) || ((scrollDirection === "down") && ($currentSection.scrollTop() + window.windowHeight >= Math.floor(currentSectionHeight / deviceZoom)))) { window.allowSlide = 1; } else { window.allowSlide = 0; } if (window.panelsToHide) { if (scrollDirection === "down" && $currentSection.scrollTop() > 0) { $body.addClass("hidePanel-top"); } else { if (scrollDirection === "up") { $body.removeClass("hidePanel-top"); } }
                    $body.addClass("hidePanel-bottom"); if (scrollDirection === "down" && $currentSection.scrollTop() + window.windowHeight >= Math.floor(currentSectionHeight / deviceZoom)) { $body.removeClass("hidePanel-bottom"); } else { if (scrollDirection === "up") { $body.addClass("hidePanel-bottom"); } } } if ((!window.sidebarShown) && (!window.popupShown) && (!window.blockScroll)) { if (window.smoothScroll) { event.preventDefault(); if (energy > 1500) { energy = 1500; } if (energy < -1000) { energy = -1500; }
                        TweenLite.to($currentSection, 0.5, { scrollTo: { y: curSecScrolltop - energy, autoKill: false }, ease: Power4.easeOut, overwrite: 5 }); } else { curSecScrolltop = (scrollDirection === "up") ? curSecScrolltop - scrollsize : curSecScrolltop + scrollsize;
                        $currentSection.scrollTop(curSecScrolltop); } } } if (window.allowSlide && scrollsize) { if (scrollDirection == "down") { window.collectScrolls = window.collectScrolls + scrollsize; } else { window.collectScrolls = window.collectScrolls - scrollsize; }
                setTimeout(function() { window.collectScrolls = 0; }, 200); } if ((Math.abs(window.collectScrolls) >= minScrollToSlide) && (window.allowSlide) && (!window.sidebarShown) && (!window.popupShown) && (!window.disableOnScroll)) { window.collectScrolls = 0; if ((scrollDirection === "down" && window.stage !== window.stages) || (scrollDirection === "up" && window.stage !== 1)) { if (window.inAction !== 1) { if (scrollDirection === "down") { window.changeSlide("increase"); } else { window.changeSlide("decrease"); } } } } } }); if ((window.isMobile && window.isSimplifiedMobile) || window.isScroll) { $(window).on("DOMMouseScroll mousewheel scroll touchmove load", function() { updateScroll(); }); } var hideDropdownOnScrollDelay = 0;

    function updateScroll() { hideDropdownOnScrollDelay++; if (hideDropdownOnScrollDelay >= 2) { hideDropdown();
            hideDropdownOnScrollDelay = 0; }
        $(".slide").each(function(index, element) { var $element = $(element),
                ...
                allowToSelect = true; if (scrollTop === 0) { if (index === 0) { allowToSelect = true; } else { allowToSelect = false; } } if (scrollTop + window.windowHeight === window.documentHeight) { if (index === window.stages - 1) { allowToSelect = true; } else { allowToSelect = false; } } if (window.setStageClasses != 0) { if (halfOnScreen && allowToSelect) { $element.prevAll(".slide").addClass("before").removeClass("after");
                    $element.nextAll(".slide").addClass("after").removeClass("before");
                    $element.addClass("selected animate active").removeClass("after before"); if ((window.stage !== elementIndex + 1) || !window.firstTimeTrigger) { window.stage = elementIndex + 1;
                        $(window).trigger("slideChange", [window.stage, $element]); if (window.stage === 1) { $body.addClass("firstSlide"); } else { $body.removeClass("firstSlide"); } if (window.stages === window.stage) { $body.addClass("lastSlide"); } else { $body.removeClass("lastSlide"); }
                        $body.removeClassByPrefix("stage-").addClass("stage-" + window.stage); if ($element.hasClass("whiteSlide")) { $body.addClass("whiteSlide"); } else { $body.removeClass("whiteSlide"); } if (window.isAnimated == "auto") { window.clearElementAnimation = setTimeout(function() { $element.find("[class*='ae-']").addClass("done"); }, window.effectSpeed + window.cleanupDelay); }
                        updateNavigation(); } if (!window.firstTimeTrigger) { window.firstTimeTrigger = 1;
                        $(window).trigger("slideChange", [window.stage, $element]); } } else { $element.removeClass("selected"); } } if ((scale > -1 && scale < 1) && !(window.allowParallaxOnMobile && window.isMobile)) { if ($element.hasClass("parallax") || $element.find(".parallax-element")) { $element.find(".parallax-element").each(function() { var $el = $(this),
                            velocity = parseInt($el.data("parallax-velocity")) ? parseInt($el.data("parallax-velocity")) : 50,
                            precentage = scale * velocity; if (velocity > 100) { velocity = 100; }
                        $el.css("-webkit-transform", "translate3d(0, " + precentage + "%, 0)").css("transform", "translate3d(0, " + precentage + "%, 0)"); }); } } }); if (window.isAnimated == "animateOnEvent") { if (!window.preload) { $("[class*='ae-']").each(function(i, element) { var $ae = $(element); if (isElementInView($ae)) { $ae.addClass("do").one("webkitAnimationEnd oanimationend msAnimationEnd animationend", function() { $(this).removeClassByPrefix("ae-").removeClass("do").addClass("done"); }); } }); } else { if (window.loaded) { $("[class*='ae-']").each(function(i, element) { var $ae = $(element); if (isElementInView($ae)) { $ae.addClass("do").one("webkitAnimationEnd oanimationend msAnimationEnd animationend", function() { $(this).removeClassByPrefix("ae-").removeClass("do").addClass("done"); }); } }); } } } }

    function isElementInView(element) { var pageTop = $(window).scrollTop(),
            $element = $(element),
            elementHeight = $element.height(),
            pageBottom = pageTop + window.windowHeight,
            elementTop = $element.offset().top,
            elementBottom = elementTop + elementHeight; if (elementHeight >= window.windowHeight / 5) { return (pageBottom >= elementTop + elementHeight / 5); } else { return ((pageTop < elementTop) && (pageBottom > elementBottom)); } }
    $(".mobile .slides:not(.scroll):not(.simplifiedMobile), .slides.desktopSwipe").swipe({ swipeStatus: function(event, phase, direction, distance) { window.allowSwipeUp = 1;
            window.allowSwipeDown = 1; var $currentSection = $(".slide.selected .content"),
                currentSectionHeight = Math.floor($currentSection.find(".container").outerHeight()),
                next = "up",
                prev = "down",
                minSwipeToSlide = window.minSwipeToSlide,
                windowHeight = window.innerHeight; if (window.sidebarShown) { $currentSection = $(".sidebar .content"); } if (window.popupShown) { $currentSection = $(".popup .content"); } if (phase === "start") { window.scrollTop = $currentSection.scrollTop(); } if (window.horizontalMode) { next = "left";
                prev = "right"; } if (!window.horizontalMode && (currentSectionHeight > windowHeight)) { if (window.scrollTop + windowHeight < currentSectionHeight) { window.allowSwipeUp = 0; } if (window.scrollTop > 0) { window.allowSwipeDown = 0; } } if (!window.sidebarShown && !window.disableOnSwipe) { if (window.horizontalMode) { if (direction === next && distance > minSwipeToSlide) { window.changeSlide("increase"); } else { if (direction === prev && distance > minSwipeToSlide) { window.changeSlide("decrease"); } } } else { if (direction === next && distance > minSwipeToSlide && window.allowSwipeUp && window.allowSlide) { window.changeSlide("increase"); } else { if (direction === prev && distance > minSwipeToSlide && window.allowSwipeDown && window.allowSlide) { window.changeSlide("decrease"); } } } } }, maxTimeThreshold: 0, fingers: "all", allowPageScroll: "vertical" });
    $(".slides.desktopSwipe *").on("click", function() { $(this).addClass("selectable"); }); if ($(".panel .compact").length > 0) { $(".panel .compact").each(function(index, element) { var panel = $(element).parents(".panel"),
                desktop = $(panel).find(".desktop"),
                compact = element,
                forceMobileView = $(panel).hasClass("forceMobileView");
            $(window).on("load resize ready", function() { var documentWidth = $(document).width(),
                    panelsPadding = parseInt($(panel).css("padding-left").replace("px", "")) + parseInt($(panel).css("padding-right").replace("px", "")); if ((window.isMobile || $(document).width() < 767) && forceMobileView) { $(desktop).addClass("hidden");
                    $(compact).removeClass("hidden"); } else { $(desktop).removeClass("hidden");
                    $(compact).addClass("hidden"); var totalWidth = 0;
                    desktop.children().each(function() { if ($(this).outerWidth() > $(this).children().outerWidth()) { totalWidth += Math.round($(this).outerWidth()); } else { totalWidth += Math.round($(this).children().outerWidth()); } }); if ((totalWidth + Math.round(panelsPadding) > documentWidth + 2) || ((window.isMobile || documentWidth < 767) && forceMobileView)) { $(desktop).addClass("hidden");
                        $(compact).removeClass("hidden"); } else { $(desktop).removeClass("hidden");
                        $(compact).addClass("hidden"); } } }); }); } if ($(".panel.hideOnScroll").length > 0) { window.panelsToHide = true; if (window.isScroll || window.isSimplifiedMobile) { var lastScrollTop, i = 0,
                sensitivity = window.hideOnScrollSensitivity ? window.hideOnScrollSensitivity : 100,
                panelToHide = $(".panel.hideOnScroll");
            $(window).on("mousewheel", function(event) { var scrollTop = $(this).scrollTop(),
                    $panelToHide = $(panelToHide),
                    scrollSize = Math.ceil(Math.abs(event.deltaY) * event.deltaFactor); if (scrollTop > lastScrollTop) { i += scrollSize; if (i >= sensitivity) { $panelToHide.addClass("hide");
                        i = sensitivity; } } else { i -= scrollSize; if (i <= sensitivity / 5) { i = 0;
                        $panelToHide.removeClass("hide"); } }
                lastScrollTop = scrollTop; if ((scrollTop + window.windowHeight + sensitivity >= window.documentHeight) || (scrollTop + sensitivity <= 0)) { $panelToHide.removeClass("hide"); } }); } }
    $(document).on("keydown", function(e) { var delta = 2.5,
            scrollTime = 0.3,
            scrollDistance = 50,
            $currentSection = $(".slide.selected .content"),
            scrollTop = $currentSection.scrollTop(),
            finalScroll = scrollTop + parseInt(delta * scrollDistance); if (window.window.disableKeyNavigation || e.target.nodeName.toLowerCase() == "input" || e.target.nodeName.toLowerCase() == "textarea") { return; } if (e.keyCode === 37) { e.preventDefault(); if (window.horizontalMode) { window.changeSlide("decrease"); } } if (e.keyCode === 38) { if (!window.horizontalMode) { e.preventDefault();
                window.changeSlide("decrease"); } else { e.preventDefault();
                TweenLite.to($currentSection, window.scrollSpeed, { scrollTo: { y: finalScroll, autoKill: true }, ease: Power4.easeOut, overwrite: 5 }); } } if (e.keyCode === 39) { if (window.horizontalMode) { e.preventDefault();
                window.changeSlide("increase"); } } if (e.keyCode === 40) { if (!window.horizontalMode) { e.preventDefault();
                window.changeSlide("increase"); } else { e.preventDefault();
                TweenLite.to($currentSection, window.scrollSpeed, { scrollTo: { y: finalScroll, autoKill: true }, ease: Power4.easeOut, overwrite: 5 }); } } if (e.keyCode === 27) { hideSidebar();
            hideDropdown();
            hidePopup();
            unzoomImage(); } }); var navParent = $(".navigation"),
        navigation = $(navParent).find("ul"),
        slidesNumber = $(".slide:not(.exclude)").length; if ($(navigation).length > 0) { if ($(navigation).is(":empty")) { $(navigation).each(function(index, element) { for (var i = 1; i <= slidesNumber; i++) { var title = $(".slide:not(.exclude):eq(" + (i - 1) + ")").data("title"); if (title === undefined) { $(element).append("<li></li>"); } else { $(element).append('<li data-title="' + title + '"></li>'); } } }); }
        $(".navigation li").on("click touchend", function() { var thisIndes = $(this).index(),
                realIndex = $(".slide:not(.exclude):eq(" + thisIndes + ")").index(".slide");
            $(this).blur();
            window.changeSlide(realIndex + 1); }); if (!$(".side").hasClass("compact")) { $(window).on("load resize ready", function() { var containerHeight = window.windowHeight - 140,
                    container = $(".side").removeClass("compact").find("ul"),
                    totalWidth = 0;
                $(container).children().each(function() { totalWidth += Math.round($(this).outerHeight(true)); }); if (totalWidth > containerHeight) { $(".side").addClass("compact"); } else { $(".side").removeClass("compact"); } }); } }
    $("a[href^='#'][target!='_blank']").click(function(e) { var url = $(this).attr("href"),
            hashLink = url.split("#")[1],
            requestedElement = hashLink ? $('.slide[id="' + hashLink + '"], .slide[data-name="' + hashLink + '"]') : $(".slide:eq(0)"); if (requestedElement.length > 0) { e.preventDefault(); if (window.isMobile && window.isSimplifiedMobile || window.isScroll) { var target = requestedElement; if (target.length) { $("html,body").stop().clearQueue().animate({ scrollTop: target.position().top }, 1000); } if (window.setHashLink) { window.location.hash = hashLink; } } else { window.stage = $(".slide").index(requestedElement) + 1;
                showSlide(window.stage); }
            hideSidebar(); } });
    ...
    function updateNavigation() { setTimeout(function() { if ($(navigation).length > 0) { $(navigation).each(function(index, element) { $(element).find("li.selected").removeClass("selected"); var $selectedSlide = $(".slide.selected"),
                        ...
                        selectedIndex = $selectedSlide.index(".slide:not(.exclude)"); if (selectedIndex !== -1) { $(element).find("li").eq(selectedIndex).addClass("selected"); } else { if (parentSlide) { selectedIndex = $('.slide[data-slide-id="' + parentSlide + '"]').index(".slide:not(.exclude)");
                            $(element).find("li").eq(selectedIndex).addClass("selected"); } } }); } }, 100); }
    $(".sidebarTrigger[data-sidebar-id]").on("click", function() { var sidebarID = $(this).data("sidebar-id");
        window.showSidebar(sidebarID); });
    window.showSidebar = function(id) { var sidebarID = id,
            element = $('.sidebar[data-sidebar-id="' + sidebarID + '"]'),
            isAnimated = $(element).hasClass("animated"); if (!window.sidebarShown) { if (element.length > 0) { window.sidebarShown = 1;
                window.allowSlide = 0;
                $(element).removeClass("animate active").addClass("visible");
                $html.addClass("sidebarShown sidebar_" + sidebarID);
                $(element).find(".content").scrollTop(0); if (isAnimated) { clearTimeout(window.removeAnimationTimeout);
                    setTimeout(function() { $(element).addClass("animate active"); }, 100); } } } else { hideSidebar(); }
        hideDropdown(); };
    ...
    function hideSidebar() { if (window.sidebarShown) { $html.removeClass("sidebarShown").removeClassByPrefix("sidebar_"); var $sidebar = $(".sidebar.visible");
            ...
            window.removeAnimationTimeout = setTimeout(function() { $sidebar.removeClass("animate active").find(".done").removeClass("done"); }, 500);
            window.sidebarShown = 0;
            window.allowSlide = 1; } }
    $(document).on("mouseup touchstart", function(e) { var container = $(".sidebarShown .sidebar, .dropdownTrigger"); if (!container.is(e.target) && container.has(e.target).length === 0 && window.hideSidebarOnBodyClick) { hideSidebar(); } });
    $('.sidebar .close, .sidebar [data-sidebar-action="close"]').on("click touchstart", function() { hideSidebar(); });
    $(".popupTrigger[data-popup-id]").on("click", function() { var popupID = $(this).data("popup-id");
        window.showPopup(popupID); });
    window.showPopup = function(id) { var popupID = id,
            element = $('.popup[data-popup-id="' + popupID + '"]'),
            isAnimated = element.hasClass("animated"); if (element.length > 0) { hideSidebar();
            $(element).addClass("visible");
            $(window).trigger("popupShown"); if (isAnimated) { setTimeout(function() { $(element).addClass("animate active");
                    clearTimeout(window.clearPopupElementAnimation);
                    window.clearPopupElementAnimation = setTimeout(function() { $(element).find("[class*='ae-']").addClass("done"); }, window.effectSpeed + window.cleanupDelay); }, 100); }
            $html.addClass("popupShown popup_" + popupID);
            $(element).find(".content").scrollTop(0);
            window.allowSlide = 0; if (!window.popupShown) { window.popupShown = []; }
            window.popupShown.push(popupID); if ($(element).hasClass("autoplay")) { var $element = $(element),
                    iframe = $element.find("iframe"),
                    HTML5video = $element.find("video"); if (iframe.length > 0) { var iframeSrc = $(iframe).attr("src"),
                        symbol = (iframeSrc.indexOf("?") > -1) ? "&" : "?";
                    $(iframe).attr("src", iframeSrc + symbol + "autoplay=1"); } else { if (HTML5video.length > 0) { $(HTML5video)[0].play(); } } } }
        hideDropdown(); };

    function hidePopup(popupID) { popupID = typeof popupID !== "undefined" ? popupID : false; if ($.isArray(window.popupShown)) { var popupToHide = popupID ? popupID : window.popupShown.slice(-1)[0],
                $element = $('.popup.visible[data-popup-id="' + popupToHide + '"]'),
                iframe = $element.find("iframe"),
                video = $element.find("video"); if (iframe.length > 0) { $(iframe).each(function(n, element) { var iframeSrc = $(element).attr("src"),
                        symbol = (iframeSrc.indexOf("?autoplay") > -1) ? "?" : "&";
                    $(element).attr("src", $(element).attr("src").replace(symbol + "autoplay=1", "")); }); } if (video.length > 0) { $(video).each(function(n, element) { $(element)[0].pause();
                    $(element)[0].currentTime = 0; }); }
            clearTimeout(window.clearPopupElementAnimation);
            $element.addClass("hidePopup");
            $(window).trigger("popupHidden");
            setTimeout(function() { window.allowSlide = 1;
                $element.removeClass("visible animate active hidePopup").removeAttr("style").find(".done").removeClass("done");
                $html.removeClass("popup_" + popupToHide); if ($.isArray(window.popupShown)) { var i = window.popupShown.indexOf(popupToHide); if (i != -1) { window.popupShown.splice(i, 1); } } if (window.popupShown.length <= 0) { $html.removeClass("popupShown");
                    window.popupShown = false; } }, 500); } } if (window.hidePopupOnBodyClick) { $(document).on("click", function(e) { var container = $(".popupShown .popup .popupContent, .popupTrigger"); if (!container.is(e.target) && container.has(e.target).length === 0 && container.length > 0) { hidePopup(); } }); }
    $('.popup [data-popup-action="close"]').on("click", function() { hidePopup($(this).parents(".popup").data("popup-id")); }); if (window.setPopupHash) { $(".popupTrigger[data-popup-id]").on("click", function() { var hash = $(this).attr("data-popup-id");
            window.location.hash = "#" + hash; });
        window.setPopupHash = [];
        $(".popupTrigger").each(function() { var hash = $(this).attr("data-popup-id"); if ($.inArray(hash, window.setPopupHash) == -1) { window.setPopupHash.push(hash); } }); if ($.inArray(window.location.hash.split("#")[1], window.setPopupHash) !== -1) { setTimeout(function() { $('.popupTrigger[data-popup-id="' + window.location.hash.split("#")[1] + '"]').click(); }, 500);
            $(window).on("popupHidden", function() { if (history.pushState) { window.history.pushState("", "", location.href.split("#")[0]); } else { window.location.hash = ""; } }); } }
    $(window).on("resize load ready popupShown", function() { setTimeout(function() { equalizeElements(); }, 1); });

    function equalizeElements() { var gridEl = $(".grid.equal, .flex.equal"); if (gridEl.length) { $(gridEl).each(function(index, element) { var screenWidth = window.windowWidth,
                    ...
                    equalMobile = $(this).hasClass("equalMobile"); if ((screenWidth >= collapseWidth) || (equalMobile)) { var height = 0;
                    $(equalElement).each(function(index, el) { $(el).css("height", "auto"); if (height < $(el).outerHeight()) { height = $(el).outerHeight(); } });
                    $(element).find(".equalElement").each(function(index, el) { $(el).css("height", height + "px"); }); } else { $(equalElement).css("height", "auto"); } }); } }
    $(window).on("resize", function() { $html.addClass("resizing"); }).on("resizeEnd", function() { $html.removeClass("resizing"); }); var sliderEl = $(".slider"); if ($(sliderEl).length > 0) { $(sliderEl).each(function(index, element) { var $el = $(element),
                sliderID = $el.data("slider-id"),
                nextIndex = $el.find(".selected").index(); if (window.sliderStatus) { $html.removeClassByPrefix("slider_" + sliderID).addClass("slider_" + sliderID + "_" + nextIndex); } if ($el.hasClass("autoplay")) { var interval = ($el.data("slider-interval")) ? parseInt($el.data("slider-interval")) : 5000; var autoplay = setInterval(function() { $el.trigger("next"); }, interval); if ($el.data("slider-stoponclick") != false) { $('[data-slider-id="' + sliderID + '"]').on("mousedown touchstart", function() { clearInterval(autoplay); }); } } if ($el.hasClass("clickable") || $el.hasClass("autoplay")) { $el.on("click next", function(event) { var $el = $(this),
                        ...
                        clickTarget = event.target; if ($(clickTarget).data("slider-event") == "cancel") { return; }
                    $selected.removeClass("selected").addClass("hide").one("webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend", function() { $(this).removeClass("hide"); });
                    $nextElement.removeClass("hide").addClass("selected"); if (window.sliderStatus) { $html.removeClassByPrefix("slider_" + sliderID).addClass("slider_" + sliderID + "_" + nextIndex); } if (isAnimated) { $el.addClass("animateOnEvent");
                        $el.find("li").removeClassByPrefix("ae-").removeClass("do");
                        $el.find(".selected").each(function(index) { $(this).removeClassByPrefix("ae-").removeClass("do").addClass("ae-" + (index + 1)).addClass("do"); });
                        $(window).scroll(); } if (sliderID && $controller.length > 0) { $controller.children(".selected").removeClass("selected");
                        $controller.children("li:eq(" + nextIndex + ")").addClass("selected"); } }); } }); } var $controller = $(".controller"); if ($controller.length > 0) { var controllerSelector = $controller.data("controller-selector") ? $controller.data("controller-selector") : "li";
        $controller.on("click", controllerSelector, function() { var $controllerElement = $(this),
                ...
                isAnimated = $slider.hasClass("animated"); if (!$controllerElement.hasClass("selected")) { $selectedElement.removeClass("selected");
                $controllerElement.addClass("selected");
                $slider.children(".selected").removeClass("selected").addClass("hide").one("webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend", function() { $(this).removeClass("hide"); });
                $slider.children("li").eq(nextIndex).removeClass("hide").addClass("selected"); if (window.sliderStatus) { $html.removeClassByPrefix("slider_" + sliderID).addClass("slider_" + sliderID + "_" + nextIndex); } } if (isAnimated) { $slider.addClass("animateOnEvent");
                $slider.find(">li").removeClassByPrefix("ae-").removeClass("do");
                $slider.find(".selected").each(function(index) { $(this).removeClassByPrefix("ae-").removeClass("do").addClass("ae-" + (index + 1)).addClass("do"); });
                $(window).scroll(); } }); }
    $("[data-slider-action]").click(function() { if ($(this).data("slider-id")) { var $this = $(this),
                $desiredElement, nextIndex, sliderID = $this.data("slider-id"),
                ...
                isAnimated = $slider.hasClass("animated"); if (action === "next") { $desiredElement = $selected.nextOrFirst("li"); } else { if (action === "prev") { $desiredElement = $selected.prevOrLast("li"); } else { if (parseInt(action) || action === 0) { nextIndex = parseInt(action);
                        $desiredElement = $slider.find(">li:eq(" + nextIndex + ")"); } } }
            nextIndex = $desiredElement.index();
            $selected.removeClass("selected");
            $desiredElement.removeClass("hide").addClass("selected"); if (window.sliderStatus) { $html.removeClassByPrefix("slider_" + sliderID).addClass("slider_" + sliderID + "_" + nextIndex); } if (isAnimated) { $slider.addClass("animateOnEvent");
                $slider.find("li").removeClassByPrefix("ae-").removeClass("do");
                $slider.find(".selected").each(function(index) { $(this).removeClassByPrefix("ae-").removeClass("do").addClass("ae-" + (index + 1)).addClass("do"); });
                $(window).scroll(); } if ((sliderID) && ($controller.length > 0)) { $controller.find(".selected").removeClass("selected").addClass("hide").one("webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend", function() { $this.removeClass("hide"); });
                $controller.find(controllerSelector).eq(nextIndex).addClass("selected"); } } });
    $("[data-slider-id].autoHeight").each(function(index, element) { $(window).on("click resize load ready next", function() { var totalHeight = 0,
                el = $(element).find(".selected");
            $(element).find(".selected").children().each(function() { totalHeight += Math.round($(this).outerHeight(true)); });
    ...
    $(".dropdownTrigger").on("click", function() { showDropdown($(this)); });
    $(".dropdownTrigger.hover").hover(function() { showDropdown($(this), "hover"); });

    function showDropdown($this, $isHover) { $isHover = typeof $isHover !== "undefined" ? $isHover : false; var offset = $this.offset(),
            ...
            elementPosition = elementPosition.split(" "); if (!$isHover) { hideDropdown(); } if (elementPosition.indexOf("bottom") != -1) { offsetY = offsetY - $element.outerHeight();
            $element.removeClass("top").addClass("bottom"); } else { offsetY = offsetY + $this.outerHeight();
            $element.removeClass("bottom").addClass("top"); } if (elementPosition.indexOf("right") != -1) { offsetX = offsetX - $element.outerWidth() + $this.outerWidth();
            $element.removeClass("left center").addClass("right"); } else { if (elementPosition.indexOf("left") != -1) { $element.removeClass("right center").addClass("left"); } else { if (elementPosition.indexOf("center") != -1) { offsetX = offsetX - ($element.outerWidth() / 2) + ($this.outerWidth() / 2);
    ...
    function hideDropdown() { if (window.dropdownShown) { $html.removeClass("dropdownShown").removeClassByPrefix("dropdown_");
            window.dropdownShown = false;
            hideDropdownOnScrollDelay = 0;
            $(".dropdown.show").addClass("hide").one("webkitTransitionEnd otransitionend msTransitionEnd transitionend", function() { $(this).removeClass("show hide");
                $html.removeClass("dropdownShown").removeClassByPrefix("dropdown_"); });
            $(window).trigger("dropdownHidden"); } }
    $(window).on("resize", function() { hideDropdown(); });
    ...
    window.shareUrl = window.location.href; if ($(".share").data("url")) { window.shareUrl = $(".dropdown").data("url"); }
    window.shareText = document.title; if ($(".share").data("text")) { window.shareText = $(".dropdown").data("url"); }
    $(".share").sharrre({ enableHover: false, url: window.shareUrl, text: window.shareText, enableCounter: false, share: { twitter: true, facebook: true, pinterest: true, googlePlus: true, stumbleupon: true, linkedin: true }, buttons: { pinterest: { media: $(".dropdown").data("pinterest-image"), description: $(".dropdown").data("text") + " " + $(".dropdown").data("url") } }, template: $(".share").html(), render: function(api) { $(api.element).on("click touchstart", ".social-twitter", function() { api.openPopup("twitter"); });
            $(api.element).on("click touchstart", ".social-facebook", function() { api.openPopup("facebook"); });
            $(api.element).on("click touchstart", ".social-pinterest", function() { api.openPopup("pinterest"); });
            $(api.element).on("click touchstart", ".social-googlePlus", function() { api.openPopup("googlePlus"); });
            $(api.element).on("click touchstart", ".social-stumbleupon", function() { api.openPopup("stumbleupon"); });
            $(api.element).on("click touchstart", ".social-linkedin", function() { api.openPopup("linkedin"); });
            $(api.element).on("click touchstart", ".mail", function() { var subject = $(this).data("subject") ? $(this).data("subject") : "",
    ...
    $(".dialogTrigger[data-dialog-id]").on("click", function() { var dialogID = $(this).data("dialog-id");
        window.showDialog(dialogID); });
    window.showDialog = function(id) { var dialogID = id,
            $element = $('.dialog[data-dialog-id="' + dialogID + '"]'); if (!$element.is(":visible")) { $element.addClass("reveal").slideDown(500, function() { $(this).removeClass("reveal").removeClass("hidden"); }); } };
    $('.dialog [data-dialog-action="close"], .dialog [data-dialog-action="hide"]').on("click", function() { var $element = $(this).parents(".dialog"),
        ...
        $element.addClass("hide").slideUp(500, function() { $(this).removeClass("hide"); if (cookieAge && action == "close") { $.cookie(cookieName, cookieValue, { expires: cookieAge, path: cookiePath }); } }); });
    $(".dialog[data-set-cookie]").each(function(index, element) { var dialogID = $(element).data("dialog-id"),
            cookieName = ($(element).data("cookie-name")) ? $(element).data("cookie-name") : dialogID,
            cookieValue = ($(element).data("cookie-value")) ? $(element).data("cookie-value") : true; if ($.cookie(cookieName)) { $(element).hide(); } });
    $(".dialog [data-href]").on("click", function() { if ($(this).data("target")) { window.open($(this).data("href"), "_blank"); } else { window.location = $(this).data("href"); } });
    $(".dialog.hidden[data-dialog-delay]").each(function() { var timeoutDelay = parseFloat($(this).attr("data-dialog-delay")),
            $element = $(this); if (!isNaN(timeoutDelay)) { setTimeout(function() { $element.addClass("reveal").slideDown(500, function() { $(this).removeClass("reveal").removeClass("hidden"); }); }, timeoutDelay); } });
    $(".dialog[data-dialog-hide-delay]").each(function() { var timeoutDelay = parseFloat($(this).attr("data-dialog-hide-delay")),
            $element = $(this); if (!isNaN(timeoutDelay)) { setTimeout(function() { $element.addClass("hide").slideUp(500, function() { $(this).removeClass("hide"); }); }, timeoutDelay); } });
    $('.dialog [data-type="submit"]').click(function() { $(this).parents("form").submit(); });
    $("#contact-form, [data-ajax-form]").each(function(index, element) { $(element).ajaxForm(function() { var $ajaxForm = $(element),
                ...
                defaultClasses = $ajaxFormButton.attr("class"); if (ajaxFormButtonIsInput) { $ajaxFormButton.val(successText); } else { $ajaxFormButton.text(successText); }
            $ajaxFormButton.addClass(successClass);
            setTimeout(function() { if (ajaxFormButtonIsInput) { $ajaxFormButton.val(defaultText); } else { $ajaxFormButton.html(defaultText); }
                $ajaxFormButton.attr("class", defaultClasses);
                $ajaxForm[0].reset(); }, 4000); }); });
    $("audio[data-sound-id]").each(function(event, element) { var $element = $(element),
            musicID = $element.data("sound-id"),
            audio = $element[0],
            $soundButton = $('.soundTrigger[data-sound-id="' + musicID + '"]'); if (audio.autoplay) { $soundButton.addClass("playing"); } else { $soundButton.removeClass("playing"); } });
    $(".soundTrigger").click(function() { var musicID = $(this).data("sound-id"),
            $audio = $('audio[data-sound-id="' + musicID + '"]'),
            action = $audio.data("sound-action") ? $audio.data("sound-action") : "toggle",
            fade = (parseInt($audio.data("sound-fade")) >= 0 || $audio.data("sound-fade")) ? parseInt($audio.data("sound-fade")) : 500; if ($audio[0].paused && (action === "toggle" || action === "play")) { $audio[0].play();
            $audio.animate({ volume: 1 }, fade);
            $(this).addClass("playing"); } else { if (action === "toggle" || action === "pause") { $audio.animate({ volume: 0 }, fade, function() { $audio[0].pause(); });
...
window.onload = function() { let e = document.getElementById("mosDefaultText"),
        t = document.querySelector("#cloud"),
        n = document.getElementById("mosCloudButton");
    t.addEventListener("mouseover", function t() { e.style.display = "none", n.style.display = "block" }), t.addEventListener("mouseout", function t() { e.style.display = "block", n.style.display = "none" }); let o = document.querySelector("#iot"),
        l = document.getElementById("mosIOTButton");
    o.addEventListener("mouseover", function t() { e.style.display = "none", l.style.display = "block" }), o.addEventListener("mouseout", function t() { e.style.display = "block", l.style.display = "none" }); let s = document.querySelector("#manufacturing"),
        y = document.getElementById("mosManuButton");
    s.addEventListener("mouseover", function t() { e.style.display = "none", y.style.display = "block" }), s.addEventListener("mouseout", function t() { e.style.display = "block", y.style.display = "none" }); let d = document.querySelector("#product"),
        i = document.getElementById("mosProductButton");
    d.addEventListener("mouseover", function t() { e.style.display = "none", i.style.display = "block" }), d.addEventListener("mouseout", function t() { e.style.display = "block", i.style.display = "none" }); let u = document.querySelector("#fab"),
        a = document.getElementById("mosFabButton");
    u.addEventListener("mouseover", function t() { e.style.display = "none", a.style.display = "block" }), u.addEventListener("mouseout", function t() { e.style.display = "block", a.style.display = "none" }); let c = document.querySelector("#automation"),
        r = document.getElementById("mosMachineButton");
    c.addEventListener("mouseover", function t() { e.style.display = "none", r.style.display = "block" }), c.addEventListener("mouseout", function t() { e.style.display = "block", r.style.display = "none" }); let m = document.querySelector("#ai"),
        p = document.getElementById("mosAIButton");
    m.addEventListener("mouseover", function t() { e.style.display = "none", p.style.display = "block" }), m.addEventListener("mouseout", function t() { e.style.display = "block", p.style.display = "none" }) };